...
node {
	try {
		...
		try {
			...
			sh "${mvnHome}/bin/mvn -f maven-pom.xml --batch-mode --update-snapshots -fae -PuseJenkinsSnapshots -DskipTests -Dmaven.repo.local=${workspace}/.m2/repository clean install deploy"
