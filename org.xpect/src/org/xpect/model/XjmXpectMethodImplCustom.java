...
public class XjmXpectMethodImplCustom extends XjmXpectMethodImpl {
	...
	protected ISingleParameterParser findSingleParameterProvider(JvmFormalParameter parameter) {
		List<ISingleParameterParser> handler = newInstancesViaMetaAnnotation(parameter, ISingleParameterParser.class, SingleParameterParser.class);
		if (handler.isEmpty())
			return null;
		if (handler.size() == 1)
			return handler.get(0);
		throw new RuntimeException("Method " + parameter + " has more than one annotation that is annotated with " + SingleParameterParser.class);
	}
