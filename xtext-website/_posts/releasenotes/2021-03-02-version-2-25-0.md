...
* A regression in JUnit 5 API was fixed. It show should work well again.
* The ANTLR option `k` in the workflow works now as expected.
* `XbaseInterpreter` now correctly handles the `return;` statement.

## Enhancements
* The `Formatter2` is now flagged as API.
* The Java API for `Formmatter2` learned a new, more fancy syntax. e.g. instead of `it -> it.noSpace()` you now can write `this::noSpace`.
