...
The `IChangeSerializer` is a new API to listen to changes that are made to the EMF model of an XtextResource and serialize them back to the text. It has a number of benefits to the old `XtextDocument.modify()` approach:
...
* Facilities to hook into, e.g. for automatic update of an import section.
* More ways to optimize for a specific language and tune performance.
...
Some languages tie the model file names to the content, like the package declaration in Java. For such cases you may wanto to hook into operations on files or directories like move, rename or paste to adapt your models automatically. Based on the new `IChangeSerializer` we have implemented generic refactoring participants. You will get notified of such changes to react appropriately by registering your own `IResourceRelocationStrategy`.
