...
The body of the template is compiled into a big private `generate()` method. We skip the inferrer code here, as it is straightforward. But we had to extend the compiler to support rich strings and the new `FOR`Â loop our new control structures. This is described in the [next section](207_template.html#templates-compiler).
