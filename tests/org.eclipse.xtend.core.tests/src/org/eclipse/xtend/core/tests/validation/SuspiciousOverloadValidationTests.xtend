...
class SuspiciousOverloadValidationTest extends AbstractXtendTestCase {
	...
	protected def void assertSuspicious(CharSequence contents, String... messageParts) {
		...
		val featureCall = block.eAllContents.filter(XAbstractFeatureCall).findLast[ eContainingFeature != XbasePackage.Literals.XABSTRACT_FEATURE_CALL__IMPLICIT_RECEIVER ]
