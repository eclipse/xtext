	...
	def testGenericInterface() {
		val text = '''
			...
			class B implements I<String> {
				override String foo(String foo) {foo}
			}
		...
		text.file.assertNoIssues
		text.compile[
			assertTrue(getGeneratedCode("C").contains("String foo(final String foo)"))
			val instance = getCompiledClass("C").newInstance
			val method = getCompiledClass("C").getDeclaredMethod("foo", String)
