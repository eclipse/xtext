...
public class TestBatchCompiler {
  ...
  public void testWorkspaceConfig() {
    try {
      ...
      final IWorkspaceConfig config = this.batchCompiler.getWorkspaceConfig();
      File _file = new File("..");
      ...
      Set<? extends IProjectConfig> _projects = config.getProjects();
      final IProjectConfig project = IterableExtensions.head(_projects);
      ...
      Set<? extends ISourceFolder> _sourceFolders = project.getSourceFolders();
      ISourceFolder _head = IterableExtensions.head(_sourceFolders);
  ...
  public void testWorkspaceConfigMultipleSourceDirs1() {
    try {
      ...
      Set<? extends ISourceFolder> _sourceFolders_1 = project.getSourceFolders();
      final Function1<ISourceFolder, String> _function = new Function1<ISourceFolder, String>() {
        @Override
        public String apply(final ISourceFolder it) {
          return it.getName();
        }
      };
      Iterable<String> _map = IterableExtensions.map(_sourceFolders_1, _function);
  ...
  public void testWorkspaceConfigSameDir() {
    ...
    IWorkspaceConfig _workspaceConfig = this.batchCompiler.getWorkspaceConfig();
