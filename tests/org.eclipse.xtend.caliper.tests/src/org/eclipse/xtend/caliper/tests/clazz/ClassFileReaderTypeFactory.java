 ...
 */
class ClassFileReaderTypeFactory implements ITypeFactory<BinaryClass, JvmDeclaredType> {

	private static final BinaryTypeSignature JAVA_LANG_OBJECT = BinarySignatures
			.createTypeSignature(CharOperation.concatWith(TypeConstants.JAVA_LANG_OBJECT, '/'));
	...
	@Inject
	public ClassFileReaderTypeFactory(ClassFileReaderAccess readerAccess, ClassLoader loader) {
	...
	protected void setSuperTypes(IBinaryType classFile, BinarySuperTypeSignature genericSignature,
		...
		if (genericSignature != null) {
			...
			if (superTypes.isEmpty() && !JAVA_LANG_OBJECT.isEqualToSignature(classFile.getName())) {
	...
	protected JvmTypeReference createObjectTypeReference() {
		return createTypeReference(JAVA_LANG_OBJECT, null);
	}
