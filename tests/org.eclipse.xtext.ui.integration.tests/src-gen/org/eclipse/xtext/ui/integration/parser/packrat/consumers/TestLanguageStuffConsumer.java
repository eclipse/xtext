...
import org.eclipse.xtext.ui.integration.services.TestLanguageGrammarAccess.StuffElements;
...
public final class TestLanguageStuffConsumer extends NonTerminalConsumer {
	...
	public TestLanguageStuffConsumer(INonTerminalConsumerConfiguration configuration, ITerminalConsumer[] hiddenTokens) {
		...
		switch(entryPoint) {
			...
			case 0:
				...
				if (result!=ConsumeResult.SUCCESS) {
					error("Another token expected.", getRule().ele0KeywordStuff());
					marker.commit();
	...
	// TestLanguageGrammarAccess.INSTANCE.prStuff()
		return rule;
	}
	
	public void setRule(StuffElements rule) {
		this.rule = rule;
	}
