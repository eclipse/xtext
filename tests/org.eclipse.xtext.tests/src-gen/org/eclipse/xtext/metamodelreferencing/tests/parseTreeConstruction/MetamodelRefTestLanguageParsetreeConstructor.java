...
protected class MyRule_NameAssignment extends AssignmentToken  {
    ...
    @Override
	public IInstanceDescription tryConsume() {
		if(current.getDelegate().eClass() == grammarAccess.getMyRuleRule().getType().getClassifier())
			return tryConsumeVal();
		return null;
	}
	...
	protected IInstanceDescription tryConsumeVal() {
		if((value = current.getConsumable("name",true)) == null) return null;
		IInstanceDescription obj = current.cloneAndConsume("name");
		if(valueSerializer.isValid(obj.getDelegate(), grammarAccess.getMyRuleAccess().getNameIDTerminalRuleCall_0(), value, null)) {
			type = AssignmentType.LRC;
