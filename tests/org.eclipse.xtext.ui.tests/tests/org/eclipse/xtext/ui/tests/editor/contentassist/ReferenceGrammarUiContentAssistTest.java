...
public class ReferenceGrammarUiContentAssistTest extends AbstractContentAssistProcessorTest {
	...
	@Test public void testComputeCompletionProposalsText() throws Exception {
		newBuilder().assertText("spielplatz")
			.applyProposal().assertText("1")
			.applyProposal().assertText("\"Beschreibung\"", "{")
			.applyProposal().assertText("{")
			.applyProposal().assertText("erwachsener", "familie", "spielzeug", "kind", "}")
			.append("erwachsener ").assertText("(")
			.applyProposal().assertText("Name")
			.append("e1 ").assertText("1")
			.applyProposal().assertText(")")
			.applyProposal().append("erwachsener (e2 0) kind ").assertText("(")
			.applyProposal().assertText("Name")
			.append("k1 ").assertText("1")
			.applyProposal().assertText(")")
			.applyProposal().append("kind (k2 0) familie ").assertText("(")
			.applyProposal().assertText("keyword", "\"Name\"", "Name")
			.append("keyword ").assertText("e1","e2")
			.applyProposal().assertText("e1","e2")
