...
class ResolvedFeaturesTest extends AbstractXbaseTestCase {
	...
	val static DERVIED_CLASS_METHOD_ERASED_SIGNATURE = "derivedClassMethod(java.util.List)"
	...
	def void testDeclaredOperationsErasedSignatureAreIncludedInAllOperations() {
		...
		val resolvedOperations = typeof(DerivedClass).toResolvedOperations
		val all = resolvedOperations.getAllOperations(DERVIED_CLASS_METHOD_ERASED_SIGNATURE)
		val declared = resolvedOperations.getDeclaredOperations(DERVIED_CLASS_METHOD_ERASED_SIGNATURE)
		assertFalse(all.empty)
		assertEquals(1, declared.size)
		assertSame(declared.head, all.head)
	}
