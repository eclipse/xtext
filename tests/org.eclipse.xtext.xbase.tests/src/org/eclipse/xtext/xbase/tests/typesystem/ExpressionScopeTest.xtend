...
class ExpressionScopeTest extends AbstractXbaseTestCase {
	...
	def void testGetExpressionScope_02() {
		val block = '''{ var x = 1 }'''.expression(false) as XBlockExpression
		val expressionScope = block.resolveTypes.getExpressionScope(block, XbasePackage.Literals.XABSTRACT_FEATURE_CALL__FEATURE, ^BEFORE)
		expressionScope.containsNot('x')
	}
	...
	def void testGetExpressionScope_04() {
		val varDecl = ('''{ var x = 1 }'''.expression(false) as XBlockExpression).expressions.head
		val expressionScope = varDecl.resolveTypes.getExpressionScope(varDecl, XbasePackage.Literals.XABSTRACT_FEATURE_CALL__FEATURE, ^BEFORE)
		expressionScope.containsNot('x')
	}
	...
	def void testMemberOnIt_03() {
		...
		val expressionScope = varInit.resolveTypes.getExpressionScope(varInit, XbasePackage.Literals.XABSTRACT_FEATURE_CALL__FEATURE, ^BEFORE)
		expressionScope.contains('it')
	}
