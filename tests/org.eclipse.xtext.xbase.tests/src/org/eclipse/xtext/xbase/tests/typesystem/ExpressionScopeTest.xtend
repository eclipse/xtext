...
class ExpressionScopeTest extends AbstractXbaseTestCase {
	...
	def void testGetExpressionScope_01() {
		...
		val expressionScope = block.resolveTypes.getExpressionScope(block, XbasePackage.Literals.XABSTRACT_FEATURE_CALL__FEATURE, ^AFTER)
	...
	def void testGetExpressionScope_02() {
		val block = '''{ var x = 1 }'''.expression(false) as XBlockExpression
		val expressionScope = block.resolveTypes.getExpressionScope(block, XbasePackage.Literals.XABSTRACT_FEATURE_CALL__FEATURE, WITHIN)
		expressionScope.containsNot('x')
	}
	...
	def void testGetExpressionScope_03() {
		...
		val expressionScope = varDecl.resolveTypes.getExpressionScope(varDecl, XbasePackage.Literals.XABSTRACT_FEATURE_CALL__FEATURE, ^AFTER)
	...
	def void testGetExpressionScope_04() {
		val varDecl = ('''{ var x = 1 }'''.expression(false) as XBlockExpression).expressions.head
		val expressionScope = varDecl.resolveTypes.getExpressionScope(varDecl, XbasePackage.Literals.XABSTRACT_FEATURE_CALL__FEATURE, WITHIN)
		expressionScope.containsNot('x')
	}
	...
	def void testMemberOnIt_03() {
		...
		val expressionScope = varInit.resolveTypes.getExpressionScope(varInit, XbasePackage.Literals.XABSTRACT_FEATURE_CALL__FEATURE, ^WITHIN)
		expressionScope.contains('it')
	}
	...
	def void testReassignedType_03() {
		...
		val expressionScope = assignment.resolveTypes.getExpressionScope(assignment, XbasePackage.Literals.XABSTRACT_FEATURE_CALL__FEATURE, ^AFTER)
		expressionScope.containsNot('charAt')
		expressionScope.contains('it')
		expressionScope.containsNot('operator_lessThan')
	}
