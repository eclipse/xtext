...
public class ExpressionScopeTest extends AbstractXbaseTestCase {
  ...
  public void testGetExpressionScope_02() {
    try {
      ...
      final XBlockExpression block = ((XBlockExpression) _expression);
      IResolvedTypes _resolveTypes = this._iBatchTypeResolver.resolveTypes(block);
      final IExpressionScope expressionScope = _resolveTypes.getExpressionScope(block, XbasePackage.Literals.XABSTRACT_FEATURE_CALL__FEATURE, IExpressionScope.Anchor.WITHIN);
      this.containsNot(expressionScope, "x");
    } catch (Throwable _e) {
      throw Exceptions.sneakyThrow(_e);
    }
  ...
  public void testGetExpressionScope_04() {
    try {
      ...
      final XExpression varDecl = IterableExtensions.<XExpression>head(_expressions);
      IResolvedTypes _resolveTypes = this._iBatchTypeResolver.resolveTypes(varDecl);
      final IExpressionScope expressionScope = _resolveTypes.getExpressionScope(varDecl, XbasePackage.Literals.XABSTRACT_FEATURE_CALL__FEATURE, IExpressionScope.Anchor.WITHIN);
      this.containsNot(expressionScope, "x");
    } catch (Throwable _e) {
      throw Exceptions.sneakyThrow(_e);
    }
  ...
  public void testMemberOnIt_03() {
    try {
      ...
      final IExpressionScope expressionScope = _resolveTypes.getExpressionScope(varInit, XbasePackage.Literals.XABSTRACT_FEATURE_CALL__FEATURE, IExpressionScope.Anchor.WITHIN);
      this.contains(expressionScope, "it");
    } catch (Throwable _e) {
      throw Exceptions.sneakyThrow(_e);
    }
