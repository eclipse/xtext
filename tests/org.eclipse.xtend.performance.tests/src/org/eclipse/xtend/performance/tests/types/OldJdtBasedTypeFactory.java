 ...
 */
public class OldJdtBasedTypeFactory implements ITypeFactory<IType> {

	private final static Logger log = Logger.getLogger(OldJdtBasedTypeFactory.class);
	private final TypeURIHelper uriHelper;
	private WorkingCopyOwner workingCopyOwner;
	
	private StoppedTask resolveAnnotations = Stopwatches.forTask("resolve annotations (OldJdtBasedTypeFactory)");
	private StoppedTask resolveMembers = Stopwatches.forTask("resolve members (OldJdtBasedTypeFactory)");
	private StoppedTask resolveTypeParams = Stopwatches.forTask("resolve typeParams (OldJdtBasedTypeFactory)");
	private StoppedTask resolveBinding = Stopwatches.forTask("resolve binding (OldJdtBasedTypeFactory)");
	private StoppedTask createType = Stopwatches.forTask("JdtBasedTypeFactory.createType (OldJdtBasedTypeFactory)");

	@Deprecated
	public OldJdtBasedTypeFactory(TypeURIHelper uriHelper) {
	 ...
	 * @since 2.4
	 */
	public OldJdtBasedTypeFactory(TypeURIHelper uriHelper, WorkingCopyOwner workingCopyOwner) {
...
//			throw new IllegalStateException("Could not create binding for '" + jdtType.getFullyQualifiedName() + "'.");
		...
		for(IBinding binding: bindings) {
			if (binding instanceof ITypeBinding) {
				createType.start();
				result.add(createType((ITypeBinding) binding));
				createType.stop();
			} else {
				result.add(null);
			}
	...
	public static class ParameterNameInitializer implements Runnable {
		private final static Logger log = Logger.getLogger(OldJdtBasedTypeFactory.ParameterNameInitializer.class);
