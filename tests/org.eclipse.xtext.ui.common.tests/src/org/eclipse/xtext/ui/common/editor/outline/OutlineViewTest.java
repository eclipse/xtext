...
public class OutlineViewTest extends AbstractEditorTest {
	...
	protected void assertSynchronized(XtextEditor editor, int elementIndex, int offset, int length) {
		...
		assertTrue(document.readOnly(new SyncEditorToOutlineAndCheckSelection(editor, elementIndex, outlinePage, offset, length)));
	}
	...
	private class SyncEditorToOutlineAndCheckSelection implements UnitOfWork<Boolean> {
		...
		public SyncEditorToOutlineAndCheckSelection(XtextEditor editor, int elementIndex, IContentOutlinePage outlinePage, int offset, int length) {
			this.editor = editor;
