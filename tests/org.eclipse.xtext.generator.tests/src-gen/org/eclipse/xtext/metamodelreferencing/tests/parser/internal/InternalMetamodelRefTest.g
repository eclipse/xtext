...
(
	lv_nameRefs=
ruleNameRef {if ($current==null) {$current = factory.create("Foo");}factory.add($current, "nameRefs",lv_nameRefs);}
)*);

ruleNameRef returns [Object current=null] : {Object temp=null;}
	
(
	lv_name=
RULE_STRING {if ($current==null) {$current = factory.create("xtext::RuleCall");}factory.set($current, "name",lv_name);}
);
