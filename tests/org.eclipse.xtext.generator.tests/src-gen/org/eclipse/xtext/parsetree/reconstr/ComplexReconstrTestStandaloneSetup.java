...
public abstract class ComplexReconstrTestStandaloneSetup {
	...
	public synchronized static void doSetup() {
		if(!isInitialized) {
			...
			for (IServiceRegistration reg :  new org.eclipse.xtext.parsetree.reconstr.ComplexReconstrTestRuntimeConfig().registrations()) {
				ServiceRegistry.registerFactory(reg.scope(), reg.serviceFactory(), reg.priority());
			}
			
			// register resource factory to EMF
			IResourceFactory resourceFactory = new org.eclipse.xtext.parsetree.reconstr.services.ComplexReconstrTestResourceFactory();
