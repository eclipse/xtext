...
public final class OptionalEmptyTestLanguageGreetingConsumer extends NonTerminalConsumer {
	...
	public OptionalEmptyTestLanguageGreetingConsumer(INonTerminalConsumerConfiguration configuration, ITerminalConsumer[] hiddenTokens) {
		...
		switch(entryPoint) {
			...
			case 0:
				...
				if (result!=ConsumeResult.SUCCESS) {
					error("Another token expected.", getRule().ele0KeywordHallo());
					marker.commit();
	...
	protected int consumeKeyword$2(int entryPoint) throws Exception {
		return consumeKeyword(getRule().ele0KeywordHallo(), null, false, false, getKeyword$2$Delimiter());
	}
