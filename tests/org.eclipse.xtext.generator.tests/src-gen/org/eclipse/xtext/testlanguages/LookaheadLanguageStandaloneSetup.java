...
public abstract class LookaheadLanguageStandaloneSetup {
	...
	public synchronized static void doSetup() {
		if(!isInitialized) {
			...
			for (IServiceRegistration reg :  new org.eclipse.xtext.testlanguages.LookaheadLanguageRuntimeConfig().registrations()) {
				ServiceRegistry.registerFactory(reg.scope(), reg.serviceFactory(), reg.priority());
			}
			
			// register resource factory to EMF
			IResourceFactory resourceFactory = new org.eclipse.xtext.testlanguages.services.LookaheadLanguageResourceFactory();
