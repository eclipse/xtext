...
public class TextChangeCombinerTest {
	...
	public void testMultipleFileChanges() throws Exception {
		...
		assertTrue(combined instanceof DisplayChangeWrapper);
		Change delegateChange = ((DisplayChangeWrapper) combined).getDelegate();
		assertTextType(delegateChange);
		assertTrue(delegateChange instanceof CompositeChange);
		assertFalse(delegateChange.equals(compositeChange));
		assertEquals(1, ((CompositeChange) delegateChange).getChildren().length);
		Change combinedChild = ((CompositeChange) delegateChange).getChildren()[0];
