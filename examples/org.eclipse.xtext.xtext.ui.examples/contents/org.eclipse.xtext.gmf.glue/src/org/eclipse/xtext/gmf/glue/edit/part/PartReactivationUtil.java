...
/**
 * When closing the popup {@link XtextEditor}, the keybindings are lost. This
 * tool offers a hack to restore them.
 * 
 * @author koehnlein
 */
public class PartReactivationUtil {
	...
	/**
	 * Hack: Reactivate the given editor to restore its keybindings.
	 * Unfortunately that only works by switching to another part and back. If
	 * there is no other part, we use Java reflection to unset the active part
	 * first.
	 * 
	...
	public static void forceReactivation(IWorkbenchPart partToBeReactivated) {
		try {
			...
			IWorkbenchPart otherVisiblePart = findOtherVisiblePart(page,
					partToBeReactivated);
			if (otherVisiblePart != null) {
				page.activate(otherVisiblePart);
			} else {
				// evil reflective call of invisible method
				Method declaredMethod = page.getClass().getDeclaredMethod(
						"setActivePart", IWorkbenchPart.class, Boolean.TYPE);
	...
	private static IWorkbenchPart findOtherVisiblePart(IWorkbenchPage page,
			IWorkbenchPart partToBeReactivated) {
