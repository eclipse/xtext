 ...
 */
class CopiedResourceDescription extends AbstractResourceDescription {
	final static Logger log = Logger.getLogger(CopiedResourceDescription)
	URI uri
	ImmutableList<IEObjectDescription> exported

	new(IResourceDescription original) {
		this.uri = original.getURI()
		this.exported = ImmutableList.copyOf(Iterables.transform(original.getExportedObjects(), [IEObjectDescription from|
			if (from.getEObjectOrProxy().eIsProxy()) {
				return from
			}
			var InternalEObject result = EcoreUtil.create(from.getEClass()) as InternalEObject
	...
	override Iterable<QualifiedName> getImportedNames() {
		...
		return Collections.emptyList() // throw new UnsupportedOperationException("getImportedNames()");
	}
	...
	override Iterable<IReferenceDescription> getReferenceDescriptions() {
		...
		log.error(exception, exception)
		return Collections.emptyList() // throw new UnsupportedOperationException("getReferenceDescriptions()");
	}
