...
@Singleton public class SimpleBacktrackingBug325745TestLanguageTokenTypeProvider implements TokenTypeProvider {
	...
	private static final String[] TOKEN_NAMES = new PsiInternalSimpleBacktrackingBug325745TestLanguageParser(null).getTokenNames();

	private static final IElementType[] tokenTypes = new IElementType[TOKEN_NAMES.length];
	
	static {
		for (int i = 0; i < TOKEN_NAMES.length; i++) {
			tokenTypes[i] = new IndexedElementType(TOKEN_NAMES[i], i, SimpleBacktrackingBug325745TestLanguageLanguage.INSTANCE);
		}
	}
