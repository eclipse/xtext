...
public class FragmentTestLanguageExParserDefinition extends AbstractXtextParserDefinition {
	...
	public PsiElement createElement(ASTNode node) {
		...
		if (hasSemanticElement != null && hasSemanticElement) {
			...
			if (elementType == elementTypeProvider.getParserRuleFragmentsExElementType()) {
				return new PsiEObjectImpl(node) {};
			}
			if (elementType == elementTypeProvider.getParserRuleFragmentsEx_ParserRuleFragmentsParserRuleCallElementType()) {
				return new PsiEObjectImpl(node) {};
			}
