...
import static org.eclipse.xtext.parsetree.reconstr.idea.parser.antlr.internal.PsiInternalHiddenTokensMergerTestLanguageParser.*;
...
@Singleton public class HiddenTokensMergerTestLanguageTokenTypeProvider implements TokenTypeProvider {

	private static final String[] TOKEN_NAMES = new PsiInternalHiddenTokensMergerTestLanguageParser(null).getTokenNames();

	private static final IElementType[] tokenTypes = new IElementType[TOKEN_NAMES.length];
	
	static {
		for (int i = 0; i < TOKEN_NAMES.length; i++) {
			tokenTypes[i] = new IndexedElementType(TOKEN_NAMES[i], i, HiddenTokensMergerTestLanguageLanguage.INSTANCE);
		}
	}
