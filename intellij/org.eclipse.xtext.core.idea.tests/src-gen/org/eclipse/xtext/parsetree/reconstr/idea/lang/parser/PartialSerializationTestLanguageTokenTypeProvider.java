...
import static org.eclipse.xtext.parsetree.reconstr.idea.parser.antlr.internal.PsiInternalPartialSerializationTestLanguageParser.*;
...
@Singleton public class PartialSerializationTestLanguageTokenTypeProvider implements TokenTypeProvider {

	private static final String[] TOKEN_NAMES = new PsiInternalPartialSerializationTestLanguageParser(null).getTokenNames();

	private static final IElementType[] tokenTypes = new IElementType[TOKEN_NAMES.length];
	
	static {
		for (int i = 0; i < TOKEN_NAMES.length; i++) {
			tokenTypes[i] = new IndexedElementType(TOKEN_NAMES[i], i, PartialSerializationTestLanguageLanguage.INSTANCE);
		}
	}
