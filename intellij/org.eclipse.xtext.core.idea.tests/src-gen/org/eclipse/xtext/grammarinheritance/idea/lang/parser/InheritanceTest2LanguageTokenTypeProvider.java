...
@Singleton public class InheritanceTest2LanguageTokenTypeProvider implements TokenTypeProvider {
	...
	private static final String[] TOKEN_NAMES = new PsiInternalInheritanceTest2LanguageParser(null).getTokenNames();

	private static final IElementType[] tokenTypes = new IElementType[TOKEN_NAMES.length];
	
	static {
		for (int i = 0; i < TOKEN_NAMES.length; i++) {
			tokenTypes[i] = new IndexedElementType(TOKEN_NAMES[i], i, InheritanceTest2LanguageLanguage.INSTANCE);
		}
	}
