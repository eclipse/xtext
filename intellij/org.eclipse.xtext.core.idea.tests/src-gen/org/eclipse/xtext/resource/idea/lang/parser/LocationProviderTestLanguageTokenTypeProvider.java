...
import static org.eclipse.xtext.resource.idea.parser.antlr.internal.PsiInternalLocationProviderTestLanguageParser.*;
...
@Singleton public class LocationProviderTestLanguageTokenTypeProvider implements TokenTypeProvider {

	private static final String[] TOKEN_NAMES = new PsiInternalLocationProviderTestLanguageParser(null).getTokenNames();

	private static final IElementType[] tokenTypes = new IElementType[TOKEN_NAMES.length];
	
	static {
		for (int i = 0; i < TOKEN_NAMES.length; i++) {
			tokenTypes[i] = new IndexedElementType(TOKEN_NAMES[i], i, LocationProviderTestLanguageLanguage.INSTANCE);
		}
	}
