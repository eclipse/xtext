 ...
 */
@Ignore class ReconcilerSmokeTest extends AbstractXtendUITestCase {
...
class XbaseFormatter2 extends AbstractFormatter {
	...
	def protected void formatClosureWrapIfNeeded(XClosure expr, INode open, Collection<XExpression> children,
		...
		workbench.introManager.closeIntro(workbench.introManager.intro)
		val editor = openEditor("Foo.xtend", model)
		for(i: 0..<300) {
			editor.document.replace(model.indexOf('// editingpos')-1 + i, 0, switch i%4{
				case 0: '""'
				case 1: '\n\n'
				case 2: '\n\t\n'
				case 3: '" "'
			})
			val waitTime = (200 + Math::random * 1000) as long
