...
class XbaseFormatter extends AbstractFormatter2 {
	...
	def protected dispatch void format(XMemberFeatureCall expr, extension IFormattableDocument format) {
		...
		var calls = new SeparatorRegions<XMemberFeatureCall, ISemanticRegion>(expr.region)
		...
		for (entry : calls) {
			...
			operator.prepend[noSpace].append[noSpace; autowrap(entry.region.length) onAutowrap = indentOnce]
