...
/**
 * Concrete implementation that is used by the {@link ITypeReferenceOwner} to
 * provide instances of {@link LightweightTypeReference}. Clients may override
 * and configure a {@link StandardTypeReferenceOwner} with a custom implementation.
 * 
...
.	 * Creates a new owned converter with proper substitution of unbound wildcards.
				...
				protected void doVisitParameterizedTypeReference(ParameterizedTypeReference reference) {
					...
					if (rawType.eClass() == TypesPackage.Literals.JVM_GENERIC_TYPE) {
						...
						for(int i = 0, size = typeParameters.size(); i < size; i++) {
							...
							reference.addTypeArgument(new ParameterizedTypeReference(owner, typeParameter));
						}
					}
				}
			});
	...
	public LightweightTypeReference toRawLightweightReference(JvmTypeReference reference) {
		JvmType type = reference.getType();
		if (type != null) {
			return toRawLightweightReference(type);
		}
		return toLightweightReference(reference).getRawTypeReference();
	}
