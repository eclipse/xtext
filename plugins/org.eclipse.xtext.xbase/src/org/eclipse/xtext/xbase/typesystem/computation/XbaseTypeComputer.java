...
public class XbaseTypeComputer implements ITypeComputer {
		...
		OUTER: for(ITypeExpectation expectation: state.getExpectations()) {
			...
			if(!listTypeCandidates.isEmpty()) {
				...
				if (commonListType != null) {
					...
					commonElementType = normalizeFunctionTypeReference(commonElementType);
					expectation.acceptActualType(createCollectionTypeReference(listType, commonElementType, expectedType), ConformanceFlags.UNCHECKED);
					for (XExpression element : literal.getElements()) {
		...
		for(ITypeExpectation expectation: state.getExpectations()) {
			...
			if(commonSetType != null) {
				...
				if(!(expectedType != null && expectedType.isType(Set.class)) && commonElementType.getType() == pairType) {
					...
					commonElementType = normalizeFunctionTypeReference(commonElementType);
					expectation.acceptActualType(createCollectionTypeReference(setType, commonElementType, expectedType), ConformanceFlags.UNCHECKED);
					for (XExpression element : literal.getElements()) {
						state.refineExpectedType(element, commonElementType);
					}
				}
	 ...
	 */
	private LightweightTypeReference normalizeFunctionTypeReference(LightweightTypeReference collectionElementType) {
