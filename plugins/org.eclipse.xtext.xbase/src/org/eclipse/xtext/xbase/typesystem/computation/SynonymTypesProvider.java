...
public class SynonymTypesProvider {
	...
	public static abstract class Acceptor {
		...
		protected final boolean accept(LightweightTypeReference synonym, EnumSet<ConformanceHint> hints) {
			return accept(synonym, ConformanceHint.toFlags(hints));
		}
		
		protected abstract boolean accept(LightweightTypeReference synonym, int hints);
	...
	public void collectSynonymTypes(/* @Nullable */ LightweightTypeReference type, /* @NonNull */ Acceptor acceptor) {
		...
		if (type.isWrapper()) {
			if (!acceptor.accept(type.getPrimitiveIfWrapperType(), ConformanceHints.CHECKED_SUCCESS | ConformanceHints.UNBOXING)) {
				return;
			}
			...
			if (!acceptor.accept(type.getWrapperTypeIfPrimitive(), ConformanceHints.CHECKED_SUCCESS | ConformanceHints.BOXING)) {
				return;
			}
			// a primitive type is never an array or list
			collectCustomSynonymTypes(type, acceptor);
		...
		if (type.isArray()) {
			...
			if (listType != null) {
				...
				if (componentType.isPrimitive()) {
					if (!acceptor.accept(listType, ConformanceHints.CHECKED_SUCCESS | ConformanceHints.DEMAND_CONVERSION | ConformanceHints.BOXING)) {
						return false;
					}
				} else {
					if (!acceptor.accept(listType, ConformanceHints.CHECKED_SUCCESS | ConformanceHints.DEMAND_CONVERSION)) {
						return false;
					}
				}
			}
			...
			if (arrayType != null) {
				...
				if (primitiveComponentType != componentType) {
					...
					if (!acceptor.accept(primitiveArray, ConformanceHints.CHECKED_SUCCESS | ConformanceHints.DEMAND_CONVERSION | ConformanceHints.UNBOXING)) {
						return false;
					}
				}
				if (!acceptor.accept(arrayType, ConformanceHints.CHECKED_SUCCESS | ConformanceHints.DEMAND_CONVERSION)) {
					return false;
				}
			}
		}
	 ...
	 * Announce a synonym type with the given conformance hint.
	 */
	protected final boolean announceSynonym(LightweightTypeReference synonym, int hint, Acceptor acceptor) {
		return acceptor.accept(synonym, hint | ConformanceHints.CHECKED_SUCCESS);
	}
