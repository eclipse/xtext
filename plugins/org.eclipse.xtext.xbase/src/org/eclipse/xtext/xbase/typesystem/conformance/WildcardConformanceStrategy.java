		...
		if (param.asTypeArgument) {
			...
			if (leftLowerBound != null) {
				...
				if (rightLowerBound != null) {
					TypeConformanceResult result = conformanceComputer.isConformant(rightLowerBound, leftLowerBound, new TypeConformanceComputationArgument(false, false, false));
					if (!result.isConformant()) {
						return TypeConformanceResult.FAILED;
					}
				} else {
			...
			for(LightweightTypeReference upperBound: leftWildcard.getUpperBounds()) {
				TypeConformanceResult result = conformanceComputer.isConformant(upperBound, rightWildcard, new TypeConformanceComputationArgument(false, false, false));
				if (!result.isConformant()) {
					return result;
				}
			}
