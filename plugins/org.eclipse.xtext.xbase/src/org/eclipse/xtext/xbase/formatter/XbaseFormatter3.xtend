...
import org.eclipse.xtext.formatter.impl.Formatter
...
class XbaseFormatter3 implements Formatter.Impl {

	static class Service extends Formatter {
		@Accessors(PUBLIC_GETTER) @Inject XbaseGrammarAccess grammar

		override protected createFormatterImpl(IFormatter.Request request) {
			new XbaseFormatter3(this, request)
		}
	}

	protected extension val XbaseGrammarAccess
	protected extension val ITokenAccess

	override format(IFormattableDocument document) {
		format(document.request.tokens.resource.contents.head, document)
	}
	...
	new(XbaseFormatter3.Service service, IFormatter.Request request) {
		this._xbaseGrammarAccess = service.grammar
		this._iTokenAccess = request.tokens
	}

	def protected dispatch void format(XCollectionLiteral literal, extension IFormattableDocument document) {
		literal.tokenForKeyword('#').append[noSpace]
		val open = literal.tokenForKeyword("[") ?: literal.tokenForKeyword("{")
		val close = literal.tokenForKeyword("]") ?: literal.tokenForKeyword("}")
	...
	def protected formatCommaSeparatedList(
		...
		if (elements.empty) {
			open.append[noSpace]
		} else if (close.leadingGap.multiline) {
			...
			val offset = head.leadingGap.offset
			val lenght = head.trailingGap.offset - offset
			formatConditionally(offset, lenght,
				...
				[ extension it |
					if (head.prependNewLineIfMultiline)
						head.prepend[newLine; increaseIndentation]
					else
						head.prepend[noSpace]
					head.format(it)
				])
			for (arg : elements.tail) {
				val argoffset = arg.immediatelyPrecedingKeyword(",").leadingGap.offset
				val arglenght = arg.trailingGap.offset - argoffset
				formatConditionally(argoffset, arglenght,
					[ doc |
						val extension it = doc.requireFitsInLine
					...
					[ extension it |
						...
						if (arg.prependNewLineIfMultiline)
							comma.append[newLine; increaseIndentation]
						else
	...
	def protected dispatch void format(XAnnotation ann, extension IFormattableDocument document) {
		ann.tokenForKeyword("@").append[noSpace]
		ann.tokenForKeyword("(").surround[noSpace]
		if (ann.value != null) {
			ann.value.format(document)
			ann.tokenForKeyword(")").prepend[noSpace]
		} else if (!ann.elementValuePairs.empty) {
			for (pair : ann.elementValuePairs) {
				pair.tokenForKeyword("=").surround[noSpace]
	...
	def protected formatArrayBrackets() {
		[ ITextReplacer.Context it |
			val t = text
	...
	def protected dispatch void format(JvmGenericArrayTypeReference array, extension IFormattableDocument document) {
		val a = formatArrayBrackets
		array.tokenForRuleCallTo(arrayBracketsRule).setReplacer(a)
	...
	def protected dispatch void format(XFunctionTypeRef func, extension IFormattableDocument document) {
		func.tokenForKeyword("(").append[noSpace]
		for (param : func.paramTypes) {
			param.format(document)
			param.immediatelyFollowingKeyword(",").prepend[noSpace].append[oneSpace]
		}
		func.tokenForKeyword(")").prepend[if(!func.paramTypes.empty) noSpace].append[noSpace]
		func.tokenForKeyword("=>").append[noSpace]
	...
	def protected dispatch void format(JvmWildcardTypeReference ref, extension IFormattableDocument document) {
		if (!ref.constraints.empty)
			ref.tokenForKeyword("?").append[oneSpace]
		for (c : ref.constraints)
			c.format(document)
	}
	...
	def protected dispatch void format(XVariableDeclaration expr, extension IFormattableDocument format) {
		expr.tokenForKeyword("val").append[oneSpace]
		expr.tokenForKeyword("var").append[oneSpace]
		expr.type.append[oneSpace]
		expr.tokenForKeyword("=").surround[oneSpace]
	...
	def protected dispatch void format(XAssignment expr, extension IFormattableDocument format) {
		expr.tokenForRuleCallTo(opSingleAssignRule).surround[oneSpace]
		expr.tokenForKeyword(if(expr.explicitStatic) '::' else '.').surround[noSpace]
	...
	//		}
	//	}
	def protected void formatFeatureCallParams(List<XExpression> params, IToken open, IToken close,
		extension IFormattableDocument format) {
	...
	def protected void formatBuilderWithLeadingGap(XClosure closure, extension IFormattableDocument format) {
		if (closure != null) {
			val offset = closure.leadingGap.offset
			val length = closure.trailingGap.offset - offset
	...
	def protected dispatch void format(XConstructorCall expr, extension IFormattableDocument format) {
		expr.tokenForFeature(XCONSTRUCTOR_CALL__CONSTRUCTOR).prepend[oneSpace]
		if (!expr.typeArguments.empty) {
			expr.tokenForKeyword("<").surround[noSpace]
			for (arg : expr.typeArguments) {
				arg.format(format)
				arg.immediatelyFollowingKeyword(",").prepend[noSpace].append[oneSpace]
			}
			expr.tokenForKeyword(">").prepend[noSpace]
		}
		val open = expr.tokenForKeyword("(")
		val close = expr.tokenForKeyword(")")
	...
	def protected void formatFeatureCallTypeParameters(XAbstractFeatureCall expr, extension IFormattableDocument format) {
		if (!expr.typeArguments.empty) {
			expr.tokenForKeyword("<").append[noSpace]
			for (arg : expr.typeArguments) {
				arg.format(format)
				arg.immediatelyFollowingKeyword(",").prepend[noSpace].append[oneSpace]
			}
	...
	def protected dispatch void format(XFeatureCall expr, extension IFormattableDocument format) {
		...
		if (expr.explicitOperationCall) {
			val open = expr.tokenForKeyword("(").prepend[noSpace]
			val close = expr.tokenForKeyword(")")
	...
	def protected dispatch void format(XMemberFeatureCall expr, extension IFormattableDocument format) {
		...
		for (call : calls.reverse) {
			...
			call.tokenForKeyword(op).prepend[noSpace].append[noSpace; autowrap]
			if (call.explicitOperationCall) {
				val open = call.tokenForKeyword("(").prepend[noSpace]
				val close = call.tokenForKeyword(")")
	...
	def protected AbstractRule binaryOperationPrecedence(EObject op) {
		val node = op.tokenForFeature(XABSTRACT_FEATURE_CALL__FEATURE)
	...
	def protected dispatch void format(XBinaryOperation expr, extension IFormattableDocument format) {
		...
		for (call : calls.reverse) {
			val op = call.tokenForFeature(XABSTRACT_FEATURE_CALL__FEATURE)
			val offset = op.leadingGap.offset
			val length = call.rightOperand.trailingGap.offset - offset
			formatConditionally(offset, length,
				[ doc |
					val extension it = doc.requireFitsInLine
				...
				[ extension it |
					...
					if (call.rightOperand.prependNewLineIfMultiline)
						op.append[newLine; increaseIndentation]
					else
						op.append[oneSpace]
					format(call.rightOperand, it)
				])
	...
	def protected dispatch void format(XSynchronizedExpression expr, extension IFormattableDocument format) {
		if (expr.eContainer instanceof XVariableDeclaration) {
			expr.tokenForKeyword("synchronized").append[increaseIndentation]
			expr.append[decreaseIndentation]
		}
		val multiline = expr.expression.multiline || expr.expression.leadingGap.multiline
		expr.param.surround[noSpace]
		if (expr.expression instanceof XBlockExpression || multiline)
			expr.tokenForKeyword("synchronized").append(whitespaceBetweenKeywordAndParenthesisML)
		else
			expr.tokenForKeyword("synchronized").append(whitespaceBetweenKeywordAndParenthesisSL)
	...
	def protected dispatch void format(XIfExpression expr, extension IFormattableDocument format) {
		if (expr.eContainer instanceof XVariableDeclaration) {
			expr.tokenForKeyword("if").append[increaseIndentation]
			expr.append[decreaseIndentation]
		}
		...
		if (expr.then instanceof XBlockExpression || multiline)
			expr.tokenForKeyword("if").append(whitespaceBetweenKeywordAndParenthesisML)
		else
			expr.tokenForKeyword("if").append(whitespaceBetweenKeywordAndParenthesisSL)
	...
	def protected dispatch void format(XForLoopExpression expr, extension IFormattableDocument format) {
		expr.tokenForKeyword("for").append[oneSpace]
	...
	def protected dispatch void format(XBasicForLoopExpression expr, extension IFormattableDocument format) {
		expr.tokenForKeyword("for").append[oneSpace]
		expr.tokenForKeyword("(").append[noSpace]
		expr.tokensForKeywords(";").forEach[prepend[noSpace].append[noSpace lowPriority]]
		expr.tokensForKeywords(",").forEach[prepend[noSpace].append[oneSpace]]
		expr.tokenForKeyword(")").prepend[noSpace]
	...
	def protected dispatch void format(XWhileExpression expr, extension IFormattableDocument format) {
		expr.tokenForKeyword("while").append(whitespaceBetweenKeywordAndParenthesisML)
	...
	def protected dispatch void format(XTypeLiteral expr, extension IFormattableDocument format) {
		expr.tokenForKeyword("typeof").append[noSpace]
		expr.tokenForFeature(XTYPE_LITERAL__TYPE).prepend[noSpace].append[noSpace]
		val a = formatArrayBrackets
		expr.tokensForRuleCallsTo(arrayBracketsRule).forEach[append[noSpace].setReplacer(a)]
	...
	def protected dispatch void format(XCatchClause expr, extension IFormattableDocument format) {
		expr.tokenForKeyword("catch").append(whitespaceBetweenKeywordAndParenthesisML)
	...
	def protected dispatch void format(XSwitchExpression expr, extension IFormattableDocument format) {
		...
		val open = expr.tokenForKeyword("{")
		val close = expr.tokenForKeyword("}")
		expr.tokenForKeyword("switch").append[oneSpace]
		if (switchSL) {
			...
			if (expr.^default != null) {
				expr.tokenForKeyword("default").append[noSpace]
			...
			if (expr.^default != null) {
				expr.tokenForKeyword("default").prepend[newLine].append[noSpace]
		...
		for (c : expr.cases) {
			if (c.typeGuard != null && c.^case != null) {
				val typenode = c.tokenForFeature(XCASE_PART__TYPE_GUARD)
	...
	def protected IToken formatClosureParams(XClosure expr, IToken open, extension IFormattableDocument format,
		(IGapFormatting)=>void init) {
		if (expr.explicitSyntax) {
			...
			open.append(init)
			val last = expr.tokenForFeature(XCLOSURE__EXPLICIT_SYNTAX)
	...
	def protected dispatch void format(XClosure expr, extension IFormattableDocument format) {
		val open = expr.tokenForKeyword("[") ?: expr.immediatelyPrecedingKeyword("(")
		val close = expr.tokenForKeyword("]") ?: expr.immediatelyFollowingKeyword(")")
		val children = switch x:expr.expression {
			XBlockExpression: x.expressions
			default: newArrayList(x)
		}
		if (children.empty) {
			if (open.trailingGap.containsComment)
			...
			val offset = open.leadingGap.nextToken.endOffset
			val length = close.trailingGap.offset - offset
			formatConditionally(offset, length,
				[ doc |
					val extension it = doc.requireFitsInLine
	...
	def protected void formatExpressionsMultiline(Collection<? extends XExpression> expressions, IToken open,
		IToken close, extension IFormattableDocument format) {
	...
	def protected boolean isMultilineOrInNewLine(EObject obj) {
		obj != null && (obj.multiline || obj.leadingGap.multiline)
	}
