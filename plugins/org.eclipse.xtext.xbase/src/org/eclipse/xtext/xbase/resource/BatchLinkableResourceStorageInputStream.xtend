...
class BatchLinkableResourceStorageFacade extends ResourceStorageFacade {
	...
	override createResourceStorageOutputStream(OutputStream out) {
		super.createResourceStorageOutputStream(out)
	}
...
class BatchLinkableResourceStorageInputStream extends ResourceStorageInputStream {
	...
	protected def readAssociationsAdapter(BatchLinkableResource resource, ZipInputStream stream) {
		...
		sourceToTargetMap.entrySet.forEach [
			adapter.sourceToTargetMap.put(resource.getEObject(key), value.map[resource.getEObject(it)].toSet)
		]
		val targetToSourceMap = objIn.readObject as Map<String,Set<String>>
		targetToSourceMap.entrySet.forEach [
			adapter.targetToSourceMap.put(resource.getEObject(key), value.map[resource.getEObject(it)].toSet)
		]
	}
...
class BatchLinkableResourceStorageOutputStream extends ResourceStorageOutputStream {
	...
	protected def writeAssociationsAdapter(BatchLinkableResource resource, ZipOutputStream zipOut) {
		val adapter = resource.eAdapters.filter(JvmModelAssociator.Adapter).head
		if (adapter == null) {
			LOG.error("No associations in "+resource.URI, new IllegalStateException())
		} else {
			...
			for (entry : adapter.sourceToTargetMap.entrySet) {
				if (entry.key.eResource != resource) {
					...
					sourceToTarget.put(entry.key.fragment, newHashSet(entry.value.map[fragment]))
				}
			}
			...
			for (entry : adapter.targetToSourceMap.entrySet) {
				if (entry.key.eResource != resource) {
					...
					targetToSource.put(entry.key.fragment, newHashSet(entry.value.map[fragment]))
				}
			}
