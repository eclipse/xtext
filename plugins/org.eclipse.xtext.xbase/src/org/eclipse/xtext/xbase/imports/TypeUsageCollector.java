...
public class TypeUsageCollector {
	...
	private boolean isTypeLiteral(XExpression memberCallTarget) {
		if (memberCallTarget instanceof XMemberFeatureCall) {
			return ((XMemberFeatureCall) memberCallTarget).isTypeLiteral();
		}
		if (memberCallTarget instanceof XFeatureCall) {
			return ((XFeatureCall) memberCallTarget).isTypeLiteral();
		}
		return false;
	}
	...
	protected PreferredType findPreferredType(EObject owner, EReference reference, String text) {
		...
		return findPreferredType(owner, reference, referencedType, text);
	}
	...
	private PreferredType findPreferredType(EObject owner, EReference reference, JvmDeclaredType referencedType, String text) {
		...
		if(parseResult != null) {
			...
			if (preferredType != null) {
				if (preferredType.referencedType != null) {
					...
					acceptUnresolvedType(preferredType.unresolvedTypeName, refRegion);
				}
			}
		}
	}
	...
	protected void acceptUnresolvedType(String usedTypeName, ITextRegion refRegion) {
		if(currentContext != null) {
			typeUsages.addUnresolved(usedTypeName, refRegion, currentContext);
		}
	}
