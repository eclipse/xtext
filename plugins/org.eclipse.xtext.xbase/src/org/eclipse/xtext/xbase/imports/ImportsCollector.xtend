...
class ImportsCollector {
	...
	def void collectImports(XtextResource state, int offset, int endOffset, ImportsAcceptor acceptor) {
		...
		while (contents.hasNext) {
			var EObject next = contents.next()
			visitIfInRange(next, offset, endOffset, acceptor)
		}
	...
	def visitIfInRange(EObject object, int offset, int endOffset, ImportsAcceptor acceptor) {
		...
		if (region.offset >= offset && region.offset + region.length <= endOffset) {
			...
			visit(object, compositeNode.children.toList, acceptor)
		}
	}

	dispatch def void visit(JvmTypeReference semanticElement, List<INode> node, ImportsAcceptor acceptor) {
		visit(semanticElement.getType(), node, acceptor)
	}

	dispatch def void visit(Void nullCase, List<INode> node, ImportsAcceptor acceptor) {
	}

	dispatch def void visit(XAbstractFeatureCall semanticObj, List<INode> node, ImportsAcceptor acceptor) {
		collectStaticImportsFrom(semanticObj, acceptor)
	}

	dispatch def void visit(XMemberFeatureCall semanticObj, List<INode> node, ImportsAcceptor acceptor) {
	...
	dispatch def void visit(XFeatureCall semanticObj, List<INode> node, ImportsAcceptor acceptor) {
		if (semanticObj.getFeature() instanceof JvmType && semanticObj.isTypeLiteral()) {
			visit(semanticObj.getFeature() as JvmType, node, acceptor)
		} else {
			collectStaticImportsFrom(semanticObj, acceptor)
	...
	dispatch def void visit(JvmDeclaredType jvmType, List<INode> node, ImportsAcceptor acceptor) {
	...
	dispatch def  void visit(XConstructorCall semanticElement, List<INode> node, ImportsAcceptor acceptor) {
		visit(semanticElement.constructor.declaringType, node, acceptor)
	}

	dispatch def void visit(XAnnotation semanticElement, List<INode> node, ImportsAcceptor acceptor) {
		visit(semanticElement.getAnnotationType(), node, acceptor)
	}

	dispatch def void visit(XTypeLiteral semanticElement, List<INode> node, ImportsAcceptor acceptor) {
	...
	dispatch def void visit(EObject semanticObj, List<INode> node, ImportsAcceptor acceptor) {
	...
	def protected JvmIdentifiableElement getReferencedElement(EObject owner, EReference reference) {
		var JvmIdentifiableElement referencedThing = owner.eGet(reference) as JvmIdentifiableElement
