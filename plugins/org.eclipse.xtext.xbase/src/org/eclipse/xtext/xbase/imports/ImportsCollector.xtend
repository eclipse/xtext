...
class ImportsCollector {
	...
	dispatch def void visit(JvmGenericType jvmType, Iterable<INode> node, ImportsAcceptor acceptor) {
		if (jvmType.anonymous) {
			visit(jvmType.superTypes.last.type, node, acceptor)
		} else {
			_visit(jvmType as JvmDeclaredType, node, acceptor)
		}
	}
