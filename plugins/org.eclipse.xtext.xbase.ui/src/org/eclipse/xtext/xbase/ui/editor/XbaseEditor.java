...
public class XbaseEditor extends XtextEditor {
	...
	@Inject
	private ImportsAwareClipboardAction.Factory clipboardActionsFactory;
	...
	protected void createActions() {
		super.createActions();
		// replace cut/copy/paste actions with a version that can deal with imports
		ResourceBundle bundle = XbaseEditorMessages.getBundleForConstructedKeys();
		TextEditorAction action = clipboardActionsFactory.create(bundle, "Editor.Cut.", this, ITextOperationTarget.CUT); //$NON-NLS-1$
		setAction(ITextEditorActionConstants.CUT, action);
		action = clipboardActionsFactory.create(bundle, "Editor.Copy.", this, ITextOperationTarget.COPY); //$NON-NLS-1$
		setAction(ITextEditorActionConstants.COPY, action);
		action = clipboardActionsFactory.create(bundle, "Editor.Paste.", this, ITextOperationTarget.PASTE); //$NON-NLS-1$
