...
public class WhitespaceReplacer implements ITextReplacer {
	...
	private void appendIndentation(StringBuilder builder, ITextReplacerContext context, int count) {
		if (count <= 0)
			return;
		String indentation = context.getPreferences().getPreference(FormatterPreferenceKeys.indentation);
	...
	private void appendLineSeparators(StringBuilder builder, ITextReplacerContext context, int count) {
		if (count <= 0)
			return;
		String separator = context.getPreferences().getPreference(FormatterPreferenceKeys.lineSeparator);
		for (int i = 0; i < count; i++)
	...
	public ITextReplacerContext createReplacements(ITextReplacerContext context) {
		...
		if (newLineCount == 0) {
			...
			context.replaceText(region, text);
		}
