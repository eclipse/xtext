	...
	public String toString() {
		...
		for (Item item : items) {
			...
			if (offsetDigits < lenghtO)
				offsetDigits = lenghtD;
		}
		List<String> result = Lists.newArrayListWithExpectedSize(items.size());
		String format = "%0" + offsetDigits + "d %0" + lenghtDigits + "d";
		String prefix = Strings.repeat(" ", offsetDigits + lenghtDigits + 2);
		for (Item item : items) {
			if (item.region != null) {
				String region = String.format(format, item.region.getOffset(), item.region.getLength());
				result.add(region + " " + item.text);
