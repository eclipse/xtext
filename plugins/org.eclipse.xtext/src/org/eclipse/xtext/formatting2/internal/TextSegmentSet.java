...
public abstract class TextSegmentSet<T> implements Iterable<T> {
	...
	private final IdentityHashMap<T, Exception> traces = new IdentityHashMap<T, Exception>();
	...
	public IdentityHashMap<T, Exception> getTraces() {
		return traces;
	}

	@SuppressWarnings("unchecked")
	protected void handleConflict(List<T> conflicts, Exception cause) {
		List<Pair<String, ITextSegment>> items = Lists.newArrayList();
		List<Exception> causes = Lists.newArrayList();
		for (T t : conflicts) {
			items.add(Tuples.create(getTitle(t), getRegion(t)));
			causes.add(traces.get(t));
		}
		causes.add(new ConflictingRegionsException("Conflicting Regions", cause, items.toArray(new Pair[items.size()])));
		throw new MultiCauseException(causes.toArray(new Exception[causes.size()]));
	}
	...
	public String toString() {
		return new ReplacementsToString<T>().withRegionGetter(getRegionAccess()).apply(this);
	}
