...
public class TextReplacerContext implements ITextReplacerContext {
	...
	protected TextSegmentSet<ITextReplacement> createTextReplacementsSet() {
		return new ArrayListTextSegmentSet<ITextReplacement>();
	}
	...
	public int getLeadingCharsInLineCount() {
		...
		int lastOffset = replacer.getOffset();
	...
	public void replaceText(CharSequence text) {
		...
		replaceText(replacer.getOffset(), replacer.getLength(), text);
	}
	...
	public void replaceText(ITextReplacement replacement) {
		Preconditions.checkNotNull(replacer);
		if (!replacer.contains(replacement)) {
	...
	protected String toStringLocal() {
		...
		if (replacer != null)
			items.add(format("replacer=[%d-%d-%s|%s]", replacer.getOffset(), replacer.getLength(), replacer.getClass()
	...
	public ITextReplacerContext withReplacer(ITextReplacer replacer) {
		...
		while (current != null) {
			...
			if (lastReplacer != null) {
				if (lastReplacer instanceof ConditionalReplacer || lastReplacer instanceof IFormattableSubDocument) {
					Preconditions.checkArgument(lastReplacer.contains(replacer));
				} else {
					Preconditions.checkArgument(lastReplacer.getEndOffset() <= replacer.getOffset());
				}
				break;
			}
			current = current.getPreviousContext();
