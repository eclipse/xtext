 ...
 */
public abstract class AbstractTokenAccess extends AbstractTextRegion2 implements ITextRegionAccess {
	...
	public ISemanticRegion immediatelyPrecedingKeyword(EObject owner) {
		ISemanticRegion firstToken = leadingGap(owner).getNextToken();
	...
	public ISemanticRegion immediatelyPrecedingKeyword(ISemanticRegion token) {
		if (token == null)
			return null;
		ISemanticRegion previousToken = token.getPreviousToken();
	...
	public ISemanticRegion immediatelyFollowingKeyword(EObject owner, String keyword) {
		ISemanticRegion lastToken = trailingGap(owner).getPreviousToken();
	...
	public ISemanticRegion immediatelyFollowingKeyword(ISemanticRegion token, String keyword) {
		if (token == null)
			return null;
		ISemanticRegion nextToken = token.getNextToken();
		if (nextToken == null)
			return null;
	...
	public boolean isMultiline(EObject object) {
		...
		if (tokens == null)
			return false;
		ISemanticRegion current = tokens.getLeadingGap().getNextToken();
		ISemanticRegion last = tokens.getTrailingGap().getPreviousToken();
		while (current != null) {
			if (current.isMultiline())
				return true;
			if (current == last)
				return false;
			IHiddenRegion gap = current.getTrailingGap();
			if (gap.isMultiline())
				return true;
	...
	public IHiddenRegion leadingGap(EObject owner) {
	...
	public ISemanticRegion tokenForFeature(EObject owner, EStructuralFeature feat) {
	...
	public ISemanticRegion tokenForKeyword(EObject owner, String keyword) {
		EObjectTokens tokens = eObjectToTokens.get(owner);
		if (tokens == null)
			return null;
		for (ISemanticRegion token : tokens.getTokens()) {
	...
	public List<ISemanticRegion> tokensForRuleCallsTo(EObject owner, AbstractRule... rule) {
	...
	public ISemanticRegion tokenForRuleCallTo(EObject owner, AbstractRule rule) {
		EObjectTokens tokens = eObjectToTokens.get(owner);
		if (tokens == null)
			return null;
		for (ISemanticRegion token : tokens.getTokens()) {
	...
	public List<ISemanticRegion> tokensForKeywords(EObject owner, String... keywords) {
		EObjectTokens tokens = eObjectToTokens.get(owner);
		if (tokens == null)
			return null;
	...
	public IHiddenRegion trailingGap(EObject owner) {
		EObjectTokens tokens = eObjectToTokens.get(owner);
		if (tokens == null)
			return null;
