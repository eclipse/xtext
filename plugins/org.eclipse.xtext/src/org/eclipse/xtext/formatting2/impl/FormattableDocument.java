...
public abstract class FormattableDocument extends AbstractTextSegment implements IFormattableDocument {
	...
	public static class RootDocument extends FormattableDocument {
		...
		public RootDocument(IInternalFormatter formatter) {
			super();
			this.formatter = formatter;
		}

		public IInternalFormatter getFormatter() {
			return formatter;
		}

		public int getLength() {
			ITextSegment tokens = (ITextSegment) formatter.getRequest().getTextRegionAccess();
	...
	protected static class SubDocument extends FormattableDocument implements IFormattableSubDocument, ITextReplacer {
		...
		public SubDocument(int offset, int length, IFormattableDocument parent) {
			super();
			this.offset = offset;
			this.length = length;
			this.parent = parent;
	...
	protected TextSegmentSet<ITextReplacer> createTextReplacerSet() {
		return new ArrayListTextSegmentSet<ITextReplacer>();
	}
	...
	public IHiddenRegion set(IHiddenRegion hiddenRegion, Procedure1<IHiddenRegionFormatter> init) {
		if (hiddenRegion != null) {
			IInternalFormatter formatter = getFormatter();
			...
			public String apply(ITextReplacer replacer) {
				if (replacer instanceof HiddenRegionReplacer) {
					String string = new HiddenRegionFormattingToString().apply(((HiddenRegionReplacer) replacer)
							.getFormatting());
					return "[" + replacer.getText() + "|" + string + "]";
				} else if (replacer instanceof ConditionalReplacer) {
					return "[" + replacer.getText() + "|ConditionalReplacer]";
				}
