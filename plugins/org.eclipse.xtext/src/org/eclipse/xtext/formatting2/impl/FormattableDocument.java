...
public abstract class FormattableDocument extends AbstractRegion implements IFormattableDocument {
	...
	protected static class ReplacerContext extends AbstractRegion implements ITextReplacerContext,
		...
		public int getLeadingCharsInLineCount() {
			ITextRegionAccess access = getTokenAccess();
	...
	public static class RootDocument extends FormattableDocument {
		...
		public RootDocument(IFormatterRequest request, int initialIndentation, ITextReplacer.Factory<IHiddenRegionFormatting> factory) {
			super(request);
	...
	public <T extends EObject> T append(T owner, Procedure1<IHiddenRegionFormatting> after) {
		if (owner != null) {
			IHiddenRegion gap = getTokenAccess().trailingGap(owner);
			set(gap, after);
		}
		return owner;
	}
	...
	protected <T extends ITextRegion> RegionSet<T> createRegionSet(ITextRegion root, FormatterProblemReporter handler) {
	...
	public <T extends EObject> T prepend(T owner, Procedure1<IHiddenRegionFormatting> before) {
		if (owner != null) {
			IHiddenRegion gap = getTokenAccess().leadingGap(owner);
			set(gap, before);
		}
		return owner;
	}
	...
	public String toString() {
		...
		toString.withRegionToString(new Function<ReplacerContext, String>() {
			public String apply(ReplacerContext input) {
				...
				if (replacer instanceof HiddenRegionReplacer) {
					String string = new HiddenRegionFormattingToString().apply(((HiddenRegionReplacer) replacer).getFormatting());
