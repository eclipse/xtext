...
public class RegionToString {
	...
	protected String toString(AbstractElement ele) {
		return grammarToString.apply(ele);
	}
	...
	protected String toString(IHiddenRegion gap) {
		List<IHiddenRegionPart> whitespaceAndComments = gap.getWhitespaceAndComments();
		List<String> children = Lists.newArrayListWithExpectedSize(whitespaceAndComments.size());
		for (IHiddenRegionPart hidden : gap.getWhitespaceAndComments())
	...
	protected String toString(ITextRegion2 region) {
		String result;
	...
	protected List<ITextRegion2> toTokenAndGapList() {
		final int range = this.hightlightOrigin ? 4 : (Integer.MAX_VALUE / 2);
		ITextRegion2 first = null;
		{
			ITextRegion2 current = origin;
			for (int i = 0; i < range && current != null; i++) {
				first = current;
				if (current instanceof ISemanticRegion)
					current = ((ISemanticRegion) current).getLeadingGap();
				else if (current instanceof IHiddenRegion)
					current = ((IHiddenRegion) current).getPreviousToken();
				else if (current instanceof IHiddenRegionPart)
					current = ((IHiddenRegionPart) current).getGap();
				else
		...
		List<ITextRegion2> result = Lists.newArrayList();
		{
			ITextRegion2 current = first;
			for (int i = 0; i <= (range * 2) && current != null; i++) {
				result.add(current);
				if (current instanceof ISemanticRegion)
					current = ((ISemanticRegion) current).getTrailingGap();
				else if (current instanceof IHiddenRegion)
					current = ((IHiddenRegion) current).getNextToken();
				else
					throw new IllegalStateException();
			}
		}
	...
	public RegionToString withOrigin(ITextRegion2 origin) {
		this.origin = origin;
		return this;
	}
