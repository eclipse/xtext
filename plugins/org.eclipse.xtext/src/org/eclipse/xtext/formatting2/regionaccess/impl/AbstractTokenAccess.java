 ...
 *******************************************************************************/
package org.eclipse.xtext.formatting2.regionaccess.impl;
...
import org.eclipse.xtext.formatting2.impl.AbstractTextSegment;
import org.eclipse.xtext.formatting2.impl.RegionToString;
 ...
 */
public abstract class AbstractTokenAccess extends AbstractTextSegment implements ITextRegionAccess {
	...
	protected ISemanticRegion getAnyToken() {
	...
	public ISemanticRegion immediatelyPrecedingKeyword(ISemanticRegion token) {
		if (token == null)
			return null;
	...
	public ISemanticRegion immediatelyPrecedingKeyword(ISemanticRegion token, String keyword) {
		if (token == null)
			return null;
		ISemanticRegion previousToken = token.getPreviousSemanticRegion();
		if (previousToken == null)
	...
	public ISemanticRegion immediatelyFollowingKeyword(EObject owner, String keyword) {
		ISemanticRegion lastToken = trailingHiddenRegion(owner).getPreviousSemanticRegion();
		ISemanticRegion result = immediatelyFollowingKeyword(lastToken, keyword);
		return result;
	}

	public ISemanticRegion immediatelyFollowingKeyword(ISemanticRegion token, String keyword) {
		if (token == null)
			return null;
	...
	public String toString() {
		ISemanticRegion token = getAnyToken();
		if (token != null)
			return new RegionToString().withOrigin(token).toString();
		return "(empty)";
	}
