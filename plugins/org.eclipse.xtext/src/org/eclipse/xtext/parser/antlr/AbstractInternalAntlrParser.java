...
/**
 * Base class for Xtext's generated parsers. It is reasonable customizable by means of object composition, e.g. error
 * messages can be tailored to specific needs with an {@link ISyntaxErrorMessageProvider}, objects construction is
 * subject to an {@link IAstFactory}.
 */
public abstract class AbstractInternalAntlrParser extends Parser {
	...
	protected class UnorderedGroupErrorContext extends ParserErrorContext implements IUnorderedGroupErrorContext {
		...
		protected UnorderedGroupErrorContext(org.antlr.runtime.FailedPredicateException exception) {
			super(exception);
		}
	...
	private ILeafNode createLeafNode(Token token, EObject grammarElement) {
		...
		if (error != null)
			hadErrors = true;
		return nodeBuilder.newLeafNode(commonToken.getStartIndex(),
				commonToken.getStopIndex() - commonToken.getStartIndex() + 1, grammarElement, isHidden, error,
				currentNode);
	}
	...
	protected void insertCompositeNode(Action action) {
		ICompositeNode newCurrentNode = nodeBuilder.newCompositeNodeAsParentOf(action, currentNode.getLookAhead(),
				currentNode);
	 ...
	 * @since 2.5
	 */
	protected INode getLastConsumedNode() {
		return lastConsumedNode;
	}
}
