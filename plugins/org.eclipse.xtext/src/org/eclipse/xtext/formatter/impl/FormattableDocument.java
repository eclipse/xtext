...
import org.eclipse.xtext.formatter.ITextReplacer.Flow;
 ...
 */
public abstract class FormattableDocument extends AbstractRegion implements IFormattableDocument {

	protected static class ConditionalReplacer extends AbstractRegion implements ITextReplacer {
		...
		public ITokenAccess getTokenAccess() {
			return document.getTokenAccess();
		}
	}
	...
	protected static class MaxLineWidthDocument extends SubDocument {
		...
		@Override
		protected List<ITextReplacement> renderToTextReplacements(ITextReplacer.Context context) {
		...
		protected void validate(GapReplacer replacer) throws FormattingNotApplicableException {
			IGapFormatting formatting = replacer.getFormatting();
		...
		protected void validate(ITextReplacer replacer) throws FormattingNotApplicableException {
			if (replacer instanceof GapReplacer)
				validate((GapReplacer) replacer);
		}
	}

	protected static class ReplacerContext extends AbstractRegion implements ITextReplacer.Context,
		...
		private final int length;
		private final int offset;
		private RegionSet<ITextReplacement> replacements = null;
		...
		public int getLeadingCharsInLineCount() {
			ITokenAccess access = getTokenAccess();
		...
		public void replaceText(ITextRegion region, CharSequence text) {
	...
	public static class RootDocument extends FormattableDocument {
		...
		private final ITextReplacer.Factory<IGapFormatting> replacerFactory;

		private final Request request;

		public RootDocument(Request request, int initialIndentation, ITextReplacer.Factory<IGapFormatting> factory) {
			super(request);
		...
		public int getLength() {
			ITextRegion tokens = (ITextRegion) request.getTokens();
		...
		public ITextReplacer.Factory<IGapFormatting> getReplacerFactory() {
	...
	protected static class TextReplacement extends AbstractRegion implements ITextReplacement {
		private final int length;
		private final int offset;
		private final String replacement;
		private final ITokenAccess tokens;

		public TextReplacement(ITokenAccess tokens, int offset, int length, String text) {
			super();
			this.tokens = tokens;
			this.offset = offset;
			this.length = length;
	...
	private final RegionSet<ReplacerContext> replacers;

	protected FormattableDocument(IFormatter.Request request) {
		super();
		this.replacers = createRegionSet(this, request.getProblemHandler());
	}

	public IToken append(IToken token, Procedure1<IGapFormatting> after) {
		if (token != null) {
	...
	public <T extends EObject> T append(T owner, Procedure1<IGapFormatting> after) {
		if (owner != null) {
			IGap gap = getTokenAccess().trailingGap(owner);
			set(gap, after);
		}
		return owner;
	}
	...
	private IGapFormatting createGapFormatting(IGap gap) {
		return new GapFormatting(this, gap);
	}

	protected <T extends ITextRegion> RegionSet<T> createRegionSet(ITextRegion root, IFormatter.ProblemHandler handler) {
		return new RegionSet<T>(root, handler);
	}

	protected TextReplacement createReplacement(int offset, int lenght, String text) {
		return new TextReplacement(getTokenAccess(), offset, lenght, text);
	}
	...
	public IToken prepend(IToken token, Procedure1<IGapFormatting> before) {
		if (token != null) {
	...
	public <T extends EObject> T prepend(T owner, Procedure1<IGapFormatting> before) {
		if (owner != null) {
			IGap gap = getTokenAccess().leadingGap(owner);
			set(gap, before);
		}
		return owner;
	}
	...
	protected void set(IGap gap, Procedure1<IGapFormatting> init) {
		if (gap != null) {
			IGapFormatting formatting = createGapFormatting(gap);
	...
	public ITextRegion setReplacer(ITextRegion region, ITextReplacer replacer) {
		if (region != null) {
	...
	public IToken surround(IToken token, Procedure1<IGapFormatting> beforeAndAfter) {
		prepend(token, beforeAndAfter);
		append(token, beforeAndAfter);
		return token;
	}

	public <T extends EObject> T surround(T owner, Procedure1<IGapFormatting> beforeAndAfter) {
	...
	public String toString() {
		...
		toString.withRegionToString(new Function<ReplacerContext, String>() {
			public String apply(ReplacerContext input) {
				...
				if (replacer instanceof GapReplacer) {
					String string = new GapFormattingToString().apply(((GapReplacer) replacer).getFormatting());
