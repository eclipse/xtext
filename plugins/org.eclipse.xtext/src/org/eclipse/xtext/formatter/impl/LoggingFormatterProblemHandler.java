 ...
 */
public class LoggingFormatterProblemHandler implements IFormatter.ProblemHandler {

	private final Logger LOG = Logger.getLogger(LoggingFormatterProblemHandler.class);
	...
	public void handleMergeConflict(RegionSet<?> regionSet, ITextRegion existing, ITextRegion add) {
		ITextRegion root = regionSet.getRoot();
	...
	public void handleOutsideOfRegion(RegionSet<?> regionSet, ITextRegion region) {
		LOG.error("Outside of Region: " + region);
	}
	...
	protected String visualizeRegion(ITextRegion root, ITextRegion region) {
		String msg = format("Region offset=%d length=%d: %s", region.getOffset(), region.getLength(), region.toString());
		String content = new RegionsToString<ITextRegion>().withRoot(root)
				.withRegionToString(new Function<ITextRegion, String>() {
					public String apply(ITextRegion input) {
