...
public class ResourceStorageFacade implements IResourceStorageFacade {
   ...
   */
  public ResourceStorageInputStream getOrCreateResourceStorageInputStream(final StorageAwareResource resource) {
    try {
      ResourceSet _resourceSet = resource.getResourceSet();
  ...
  protected boolean doesStorageExist(final StorageAwareResource resource) {
    ...
    if (!_notEquals) {
      ...
      boolean _notEquals_1 = (!Objects.equal(_resourceStorageInputStream, null));
      _and = _notEquals_1;
    }
    if (_and) {
      return true;
