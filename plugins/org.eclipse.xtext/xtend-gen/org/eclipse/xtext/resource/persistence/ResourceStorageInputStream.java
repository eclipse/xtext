...
public class ResourceStorageInputStream {
  private final static Logger LOG = Logger.getLogger(ResourceStorageInputStream.class);
  ...
  protected void loadIntoResource(final StorageAwareResource resource) {
    try {
      ...
      boolean _not = (!_isIsLoadedFromStorage);
      if (_not) {
        throw new IllegalStateException("Please use StorageAwareResource#load(ResourceStorageInputStream).");
      }
      ...
      if (_t instanceof IOException) {
        ...
        ResourceStorageInputStream.LOG.error(_plus_2, e);
      } else {
