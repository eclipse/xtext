...
public class JdtBasedTypeFactory implements ITypeFactory<IType> {
	...
	public static class ParameterNameInitializer implements Runnable {
		 ...
		 * @since 2.4
		 */
		public ParameterNameInitializer(WorkingCopyOwner workingCopyOwner, JvmExecutable executable, String handleIdentifier, String[] path, String name, CharSequence signature) {
			super();
			this.workingCopyOwner = workingCopyOwner;
		...
		private void setParameterNames(IMethod javaMethod, List<JvmFormalParameter> parameters)
			...
			for (int i = 0; i < parameterNames.length; i++) {
				...
				if (size <= i) {
					// We expect this type of failure for enum constructors which have implicit arguments.
					//
//					if (type.isEnum() && javaMethod.isConstructor()) {
//						break;
//					}
					throw new IllegalStateException("unmatching arity for java method "+javaMethod.toString()+" and "+executable.getIdentifier());
