 ...
 * @since 2.8
 */
public abstract class AbstractJdtTypeProvider extends AbstractJvmTypeProvider implements IJdtTypeProvider {
	 ...
	 */
	protected AbstractJdtTypeProvider(IJavaProject javaProject, ResourceSet resourceSet,
	...
	private boolean canLink(JvmType type) {
		...
		if (resourceURI.isPlatformResource() && resourceURI.segment(1).equals(getProjectName())) {
	...
	private IType findObjectTypeInJavaProject(String topLevelType) throws JavaModelException {
		...
		return findObjectTypeInJavaProject(packageName, typeName);
	}

	protected abstract IType findObjectTypeInJavaProject(String packageName, String typeName) throws JavaModelException;
	...
	protected boolean isBuilderScope() {
	...
	private IMirror createMirror(/* @NonNull */ IType type) {
		...
		return new JdtTypeMirror(type, getJdtBasedTypeFactory(), services);
	}
	...
	public abstract AbstractJdtBasedTypeFactory getJdtBasedTypeFactory();
