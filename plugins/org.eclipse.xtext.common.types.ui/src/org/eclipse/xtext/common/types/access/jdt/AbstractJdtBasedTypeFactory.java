...
import org.eclipse.xtext.common.types.access.jdt.JdtBasedTypeFactory.EnumConstructorParameterNameInitializer;
import org.eclipse.xtext.common.types.access.jdt.JdtBasedTypeFactory.ParameterNameInitializer;
 ...
 */
public abstract class AbstractJdtBasedTypeFactory implements ITypeFactory<IType, JvmDeclaredType> {
	...
	protected AbstractJdtBasedTypeFactory(TypeURIHelper uriHelper) {
	...
	protected abstract WorkingCopyOwner getWorkingCopyOwner();
	...
	protected JvmDeclaredType createType(IType jdtType, IJavaProject javaProject) {
		...
		IBinding binding = doResolveBindings(jdtType, javaProject);
	...
	protected abstract IBinding doResolveBindings(IType jdtType, IJavaProject javaProject);
	...
	protected void enhanceExecutable(StringBuilder fqn, String handleIdentifier, String[] path, JvmExecutable result, IMethodBinding method) {
		...
		if (parameterTypes.length > 0) {
			...
			if (declaringClass.isFromSource()) {
				...
				ParameterNameInitializer initializer = jdtCompliance.createParameterNameInitializer(method, getWorkingCopyOwner(), result, handleIdentifier, path, name, signaturex);
