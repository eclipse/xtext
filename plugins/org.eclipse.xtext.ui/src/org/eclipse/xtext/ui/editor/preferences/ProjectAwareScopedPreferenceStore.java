...
/**
 * Mainly copied from {@link org.eclipse.ui.preferences.ScopedPreferenceStore}. It fixes the memory leek described in <a
 * href="https://bugs.eclipse.org/bugs/show_bug.cgi?id=239033">these</a> <a
 * href="https://bugs.eclipse.org/bugs/show_bug.cgi?id=362199">bugs</a>.
 * 
 * The ProjectAwareScopedPreferenceStore is an IPreferenceStore that uses the scopes provided in
...
public class ProjectAwareScopedPreferenceStore extends EventManager implements IPersistentPreferenceStore {
	...
	/**
	 * The storeContext is the context where values will stored with the setValue methods. If there are no
	 * searchContexts this will be the search context. (along with the "default" context)
	 */
	private IScopeContext storeContext;

	/**
	 * The searchContext is the array of contexts that will be used by the get methods for searching for values.
	 */
	...
	/**
	 * The listener on the IEclipsePreferences. This is used to forward updates to the property change listeners on the
	 * preference store.
	 */
	...
	/**
	 * The default context is the context where getDefault and setDefault methods will search. This context is also used
	 * in the search.
	 */
	...
	/**
	 * Does its best at determining the default value for the given key. Checks the given object's type and then looks
	 * in the list of defaults to see if a value exists. If not or if there is a problem converting the value, the
	...
	/**
	 * Return the preference path to search preferences on. This is the list of preference nodes based on the scope
	 * contexts for this store. If there are no search contexts set, then return this store's context.
	 * <p>
	 ...
	 *            <code>true</code> if the default context should be included and <code>false</code> otherwise
	...
	/**
	 * Return the string value for the specified key. Look in the nodes which are specified by this object's list of
	 * search scopes. If the value does not exist then return <code>null</code>.
	 * 
	 * @param key
	...
	/**
	 * Return the string value for the specified key. Look only in the node which is specified by this object as the
	 * store preferences {{@link #getStorePreferences()}.
	 * 
	 * @param key
	 * @return
	 */
	...
	/**
	 * Set the search contexts to scopes. When searching for a value the seach will be done in the order of scope
	 * contexts and will not search the storeContext unless it is in this list.
	 * <p>
