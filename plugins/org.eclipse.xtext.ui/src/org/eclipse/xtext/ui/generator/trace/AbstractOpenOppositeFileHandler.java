...
public abstract class AbstractOpenOppositeFileHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		Collection<StorageWithRegion> destinations = findDestinations(editor);
		switch (destinations.size()) {
			case 0:
				return null;
			case 1:
				openEditorAndSelect(page, destinations.iterator().next());
				return null;
			default:
				List<StorageWithRegion> selected = selectDestinations(page, destinations);
	...
	protected abstract Collection<StorageWithRegion> findDestinations(IEditorPart editor);
