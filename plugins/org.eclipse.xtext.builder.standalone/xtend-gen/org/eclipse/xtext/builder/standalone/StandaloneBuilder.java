...
import org.eclipse.xtext.builder.standalone.DisabledClusteringStrategy;
import org.eclipse.xtext.builder.standalone.DynamicResourceClusteringStrategy;
import org.eclipse.xtext.builder.standalone.IClusteringStrategy;
...
public class StandaloneBuilder {
  ...
  public boolean launch() {
    ...
    IClusteringStrategy _xifexpression = null;
    ClusteringConfig _clusteringConfig = this.getClusteringConfig();
    boolean _notEquals_2 = (!Objects.equal(_clusteringConfig, null));
    if (_notEquals_2) {
      DynamicResourceClusteringStrategy _xblockexpression = null;
      {
        StandaloneBuilder.LOG.info("Clustering configured.");
        ClusteringConfig _clusteringConfig_1 = this.getClusteringConfig();
        _xblockexpression = new DynamicResourceClusteringStrategy(_clusteringConfig_1);
      }
      _xifexpression = _xblockexpression;
    } else {
      _xifexpression = new DisabledClusteringStrategy();
    }
    final IClusteringStrategy strategy = _xifexpression;
    ...
    while (_while) {
      {
        ...
        while (_while_1) {
          {
            ...
            clusterIndex++;
            boolean _continueProcessing = strategy.continueProcessing(resourceSet, clusterIndex);
        ...
        final Function1<Resource, IResourceDescription> _function_2 = new Function1<Resource, IResourceDescription>() {
          public IResourceDescription apply(final Resource it) {
            URI _uRI = it.getURI();
            LanguageAccess _languageAccess = StandaloneBuilder.this.languageAccess(_uRI);
        ...
        final List<IResourceDescription> descriptions = ListExtensions.<Resource, IResourceDescription>map(_arrayList, _function_2);
        Iterable<IResourceDescription> _allResourceDescriptions = index.getAllResourceDescriptions();
        Iterable<IResourceDescription> _plus_6 = Iterables.<IResourceDescription>concat(_allResourceDescriptions, descriptions);
        ResourceDescriptionsData _resourceDescriptionsData = new ResourceDescriptionsData(_plus_6);
        index = _resourceDescriptionsData;
        if ((!continue_)) {
    ...
    while (_while_1) {
      {
        ...
        boolean _validate = this.validate(resources);
