...
h2. Changes in the grammar

The grammar language is fully backward compatible. You should not have to apply any changes here. 

h2. Changes in the workflow

Xtext 1.0 introduces MWE2 to describe the workflow to build the Xtext infrastructure. MWE2 is a domain-specific language developed with Xtext that completely replaces the original XML based MWE. MWE workflows will still work but are considered legacy. For a more detailed description see the "section on MWE2":#MWE2.

The packrat parser has been abandoned. Please use the AntLR version now. With that change, the generator fragments @AntlrDelegatingFragment@,  @DelegatingGeneratorFragment@, and @PackratParserFragment@ have vanished and should be replaced by @de.itemis.xtext.antlr.XtextAntlrGeneratorFragment@.

The @org.eclipse.xtext.generator.scoping.JavaScopingFragment@ should be replaced by @ImportURIScopingFragment@. 

h2. Changes in the generated code 
...
We have appended @.ui@ to the base package of the generated UI plug-in to comply with Eclipse naming conventions. The initial package's content in the @src@ folder will therefore not be deleted automatically. Even though that was not the primary purpose, this can help you copying your manual customizations one by one. Don't forget to delete the old packages when you're finished. After that, you will have to fix the UI plug-ins @MANIFEST.MF@ and @plugin.xml@. 

h2. Changes in the MANIFEST.MF and plugin.xml

* In the generated UI plug-in, remove the package exports of no longer existing packages and make sure the bundle activator points to the newly generated one.  
* The plug-ins @org.eclipse.xtext.ui.core@ and @org.eclipse.xtext.ui.common@ have been merged into a new single plug-in @org.eclipse.xtext.ui@. The same happened to the respective Java packages. 
...
h2. Changes in the src folders 

The @src@ folders are generated once, so existing code will not be overwritten. 

After re-generation, there will now be a @XStandaloneSetup@ inheriting form the generated @AbstractXStandaloneSetup@ to allow customization. 
...
After having migrated, you might want to use some of the new features. We recommend reading the sections about
