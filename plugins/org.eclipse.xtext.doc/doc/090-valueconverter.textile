...
and the concrete implementation can be registered via the runtime "Guice module":#guicecustomize.
...
which allows to declaratively register an _IValueConverter_ by means of an annotated method. 
...
available _EDataType_ that reuses the functionality of the corresponding _EFactory_.
...
implementing an own value converter. If the converter needs to know about the rule that it currently 
