...
The whole algorithm used to derive complete ecore models from Xtext grammars is described in yection "Meta-Model Inference":#metamodelInference.
...
p. using namespace URIs is typically only interesting when using common ecore models, such as Ecore itself or the UML metamodel. If you're developing the EPackage together with the DSL abut don't want to have it derived from the grammar for some reason, we suggest to use a resource URI.
...
Platform URIs are a special EMF construct, which allow for some kind of transparency between workspace projects and installed bundles. Consult EMF documentation (we recomend the book) for details.
...
p. When referring to a type somewhere in the grammar you need to qualify them using that alias (example ''another::CoolType''). We'll see later where such type references occur.
...
If you for instance want to add a rule to allow fully qualified names in addition to simple IDs, you should implement it as a "datatype rule":#Datatype_rules, instead of adding another terminal rule.
...
The implementation needs to be registered as a service (cf. "Service Framework":#Service_Framework).
...
h5(grammar_actions). Tree Rewrite Actions 
...
p. ... one would get unwanted elements in the resulting AST.
...
p. A list of all default terminals like WS can be found in section "Grammar Mixins":#Grammar_Mixins.
...
For conversion again value converters are responsible (cf. "value converters":Xtext/Documentation/ValueConverter).
...
h2(grammar_import). Importing existing Meta Models
...
h2(grammar_mixins). Grammar Mixins
...
p. Inheriting from another grammar makes the rules defined in that grammar referable. It is also possible to overwrite rules from the super grammar. 
