...
to the static _String_ field _INVALID TYPE NAME_ in the validator class.
...
in our DSL's UI project and new entries in the _plugin.xml gen_ file.

Continuing with the _INVALID TYPE NAME_ problem from the Domainmodel example we add a method with which 
