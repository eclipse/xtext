...
Although a couple of things have changed we tried to keep good things and often used things unchanged. At the same time we wanted to clean up wrong concepts and solve the main problems we and you had with oAW Xtext.
From a bird's eye view if you want to migrate an existing oAW Xtext project to TMF Xtext, you mainly just need to rename the old grammar from *.txtx to *.xtext and add two lines to the beginning of that document (see below for details). Also you might change a few keywords, but all in all this is pretty easy and we've migrated a couple of oAW Xtext projects this way.
The other aspect where lots of code might have been written for is validation. In oAW Xtext we used Xpand's Check language to define constraints on the meta model. Because of the huge amount of existing code in oAW Xtext projects, it was not an option to ask everyone to translate all the checks to a different language. So TMF Xtext still allows to define constraints using Check, although this has been one major reason for the lack of scalability. We've implemented some additional things which allow explicitely state when which check is to be evaluated (while you type, on save, or on triggering an explicit action). See the validation section below. 
Anyway, if you want to provide a nice user experience checks should run fast and  be evaluated while you type. Therfore we strongly encourage you to consider implementing validation using our declarative Java approach.
...
One exception might be linking and content assist which in oAW Xtext contain a lot of duplicate code. This is because in oAW we didn't have the notion of scopes (see below).
Because this concept wasn't there and the code to implement scopes usually is cluttered and duplicated through linking and content assist. We didn't manage to come up with a good compatibility layer.
...
However, if we have completely missunderstood the situation and your oAW Xtext project is not migratable in a reasonable amount of time, please tell us. We want to help you.  
...
While the aforementioned things allow for convenient specification of label and icon providers, outline views, content assist and linking. Xtend is interpreted and therefore hard to debug. Because of that Xpand is shipped with a special debugger facility. However the shipped debugger cannot be used since it implies that the Xtend functions have to be called from a workflow. This is not and cannot be the case for Xtext Editors.
...
In addition to these scalability problems we have designed some core APIs (e.g. scopes) around the idea of Iterables, which allows for lazy resolution of things. As Xtend does not know the concept of Iterators you'ld have to work with lists all the time. This is far more expensive than chaining Iterables through filters and transformers like we do with Google Collections in TMF Xtext.
...
Most of the APIs in TMF Xtext use polymorphic dispatching, which mimics the polymorphic dispatch known from Xtend. 
With polymorphic dispatch an ILabelProvider to be used in any Eclipse view can be written like so for instance:
...
As an alternative to the Java APIs we also considered other JVM languages. We like static typing and think it is especially important when processing typed models (which evolve heavily). That's why Groovy or JRuby were no alternative. Using Scala would have been a very good match, but we didn't want to require knowledge of Scala so we didn't use it. 
...
Apart from the first two lines the grammar languages are more or less compatible. The syntax for all the different EBNF concepts (alternatives, groups, cardinatlities) is similar.
...
The very handy String rules are still present in TMF Xtext but we generalized them so that you don't need to write the 'String' keyword in front of them and at the same time these rules can not only produce EStrings but (as the name suggests) any kind of EDatatype. The return type is inferred and if not specified EString is assumed, however you can now simply create a parser rule returning other EDatatypes (see TODO-REF).
...
So instead of using a special token which is syntactically a STRING token, the default import mechanism now looks fro EAttributes of type EString with the name 'importedURI'.
...
p. Although this changes your meta model, one usually never used this reference explicititely as it was only there to be used by the default import mechanism. So we assume and hope that changing the reference is not a big deal for you. 

h4(#differences_return_types). Return types

The syntax to ecplicitely declare the return type of a rule has changed. In oAW Xtext (where this was marked as 'experimental') the syntax was like so:

bc.. MyRule [MyType] : foo=ID;  

p. in TMF Xtext we have a keyword for this :

bc.. MyRule returns MyType : foo:ID;

p. So this is a bit more verbose, but at the same time more readable. And as you don't have to write the return type in most situations, it's good to have a more explicit, readable syntax.
...
p. it becomes something like the following in TMF Xtext :

bc.. 
  import my::pack::to::myMetaModel;
  context Type ERROR "foo" : name!=null;

p. Where my::pack::to::myMetaModel refers to the package the generated EClasses are in. In other words there's a Java class @my.pack.to.myMetaModel.Type@.
...
bc.. 
  context Class#feature ERROR "My message "+this.name+"." :
    my.complex.predicate;

p. This is of you add the name of a feature separated by a hash ('#'), in the editor only the value of the feature will be marked, not the whole object.
...
The linking has been completely redesigned. In oAW Xtext linking was done in a very naive way: To find an element one queries list of all 'visible' EObjects and than filters out what is not needed and tries to find a match by comparing the text written for the crosslink with the value returned by the id() extension. This was not only very expensive but also lacks an important abstraction: the notion of scopes.

h4(#differences_linking_sopes). The idea of scopes

In oAW Xtext one typically had to override the link_feature() extension in order to change the default linking. A typical rewritten extension called the allVisibleElements() extensions and than filters out everything which shouddn't be taken into account for the specific link. For all elements left we typically took the first element where the id() extension returned a string similar to what has been written in the text.

The code about selecting and filtering allElements() usually has been duplicated in the corresponding content assist function, so that linking an content assist are semantically in sync. If you're good (we usually were not) you externalized that pice of code and reused the same extension in content assist and linking.

In TMF Xtext we've introduced scopes and scopeproviders. Scopeproviders are responsible for creating scopes and a scope is basically a hierarchy of Lists of name->value pairs. This is where most of the semantics of linking typically goes and by implmenting an ${org.eclipse.xtext/src/org.eclipse.xtext.scoping.IScopeProvider} for your language linking and content assist will automatically be kept in sync since both rely and use the scope provider.

The default implementation is semantically mostly identical to how the default linking worked in oAW Xtext: 
# Elements which have an attribute name will be made visible by their name,
# referenced resources ca be put on the scope by using the 'importedURI'- naming convention (TODO-REF)
...
We expect the migration of linking to be very simple, if you've not changed the default semantics that much. We've already migrated a couple of projects it wasn't too hard to do so.
If you have changed linking (and also content assist) a lot, you'll have to translate the semantics to the IScopeProvider concept. This might a bit of work, but it is worth the effort as this will clean up your code base and better separate concerns. Also we're confident that there's much value in migrating to TMF Xtext. 

That said you might not have the time to do the migration and wished to have better support for migrating oAW linking to TMF linking.  In this document we tried to explain why we diceded to change the conceptual architecture and why it is not easily possible to come up with an adapter for the notion of scopes. If you don't want to do the migration yourself we (itemis AG) can do the work or help you with that. Note also that you're welcome to ask any questions in the newsgroup and that we'll try to help you as much as possible in order to get your projects migrated. 

h3(#differences_content_assist). Differences in Content Assist

In oAW Xtext content assist has been customized using Xtend. This was a very convenient thing because of the polymorphic dispatch. However in content assist one usually don't have to do extensive model computation. At least not in TMF Xtext as content assist relies on scope providers which do the complicated work.

In TMF Xtext content assist can be conveniently customize using a declarative Java API (see TODO-REF).

h3(#differences_outline_view). Differences in Outline View

In TMF Xtext the outline view does not support mutliple view points so far. This is just because we didn't manage to get this included (due to low priorization). We don't think that this is in general a bad idea, but we thought of other things as more important.
That said the default outline view is similar to the one in oAW Xtext, but the customization is again done using a declarative Java approach (see TODO-REF).

h3(#differences_label_provider). Differences in Label Provider

As mentioned previously label providers are also now implemented in Java and are an implementation of ILabelProvider (see TODO-REF).
