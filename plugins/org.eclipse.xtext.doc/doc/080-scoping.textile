...
uses the notion of class paths to define containers (jars and class folders) which contain any 
...
know about visibility constraints due to class path restriction. Rather than that, they are defined by 
the referencing language by means of so called _IContainers_: While Java might load a resource 
via _ClassLoader.loadResource()_ (i.e. using the class path mechanism), another language could load 
...
requires all elements to be on the class path, while the default _JavaProjectsState_ also allows models 
...
the whole class path or a set of paths for all models therein. When paths are given, each path entry 
