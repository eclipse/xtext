...
p. When enumerating the scope's content, the first, most specialized scope would return @Bar@, it's parent would provide @Stuff@ and the outermost scope adds @Foo@. The linker will iterate the scope in that order and abort when it finds a matching @ScopedElement@. 
