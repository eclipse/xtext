...
p. A single @IScope@ represents an element of a linked list of scopes. That means that a scope can be nested within an outer scope.
...
#  ${org.eclipse.xtext/src/org.eclipse.xtext.index.impl.AbstractDeclarativeIndexBasedScopeProvider}

h3. IndexBased Scoping

The index based scoping is based on qualified names and namespaces. As the name suggests it uses the EMF index to find any EObjects which are not located in the current resource. 
The ${org.eclipse.xtext/src/org.eclipse.xtext.index.impl.IndexBasedScopeProvider} supports nested namespaces (similar to C# and Scala) and is used in the Domainmodel example (project org.eclipse.xtext.example.domainmodel). There is support for declarative overwriting of the default semantics if you subclass ${org.eclipse.xtext/src/org.eclipse.xtext.index.impl.AbstractDeclarativeIndexBasedScopeProvider}.
