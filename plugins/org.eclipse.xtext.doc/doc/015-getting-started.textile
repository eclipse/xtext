...
"Download and install the latest version of Xtext":http://www.eclipse.org/Xtext/download/, 
...
and can be materialized into your workspace using the example wizard. The following instance 
of the domain model DSL should give you an idea about the language.
...
snippets from this document to your Eclipse editor, your documentation
viewer most likely has inserted characters different from {space} into your 
clipboard. Reenter these "fillers" or type the text by hand to be sure everything
works fine. 
...
p. In Xtext each grammar has a unique name, which like (most) Java classes 
needs to reflect the location of the file within the Java class path. In our 
...
whitespace, which are ignore by default. You can open that grammar in the 
editor, so you see which rules. It turns out that these set of rules are often 
...
Also you can use the grammar inheritance mechanism for your grammar libraries, 
of course.
...
tree, since it also contains cross links. But as the term AST is so commonly 
used and known we are ignoring this little detail. 
...
name and namespace URI (short: _nsURI_), so we have to declare these two values 
in that statement. 
...
Bi-directional because Xtext not only derived a parser from the grammar but also 
...
p. The assignment adds (the '_+=_' operator) every object returned by the 
_Entity_ rule to the _elements_ referenced of the AST type _Model_. If you use '_+=_' 
as opposed to '_=_' the feature needs to be a list (upperBound=-1). In addition to the 
...
side was parsed. Here's the list of available assignment operators:

|| _foo=..._   | corresponds to _setFoo(...)_  ||
|| _foo+=..._  | corresponds to _getFoo().add(...)_||
|| _foo?=..._  | corresponds to _setFoo(true)_||
...
p. The question mark marks an element or as in this case an entire group optional.
...
postfix '*' (technically a keyword) should trigger a boolean flag in the AST model. 
...
reference entities but all instances of type. So far this only includes data type, but 
you may want to introduce additional concepts in future version, like for instance a 
...
Now that we have the grammar in place and defined we need to run the generator. 
...
If code generation succeeded, we are now able to test the IDE integration. 
...
languages invent concepts like name spaces or modules. We will now extend the language 
by the concept of a _Package_, which effectively is a name space. 

Our languages's definition of a _Package_ is as follows: A package has a name and 
contains other named elements and name space imports. A named element is either another 
...
_qualified names_ which are relative to the common namespaces. By means 
...
Before we extend the grammar, let's have a look at how a the example model would
...
often cause problems). Instead we want it to be a parser rule, but one which
doesn't return an instance of an _EClass_ from the referenced _EPackage_ but 
just return a String. Such rules are called _data type rules_, because they return 
...
rule_ rather then a normal parser rule, because it doesn't contain any assignments 
and all rule calls go to either other _data type rules_ or other _terminal rules_. 
...
called _ValueConverters_. But as we use strings we don't need to care about 
...
p. An imported name space is not just a qualified name but it also allows an 
