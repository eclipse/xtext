...
In this mini-tutorial you will implement your first language and a corresponding Eclipse IDE integration with Xtext. Later, we will create a code generator that is capable of reading the models you create with the DSL editor and process them, i.e. generate some Java code from it.
...
define domain models. It is stripped-down version of the _Xtext Domain-Model Example_ which is shipped with Xtext and can be materialized into your workspace using 
...
In order to get started we first need to create some Eclipse projects. Use the Xtext wizard to for that:
...
also sometimes refer to it as _semantic model_ or _abstract syntax tree (AST)*. Note, that in the case of Xtext it's actually a graph rather than a tree, since it also 
...
We want to allow inheritance for entities and therefore now add an optional 'extend' part.
...
The parser only parses the name of the cross referenced element using the ID rule and stores it internally. Later on the linker establishes the cross reference using the name
, the defined cross reference's type (_Entity_ in this case) and the defined scoping rules. Scoping is not covered in this tutorial since the domain model DSL works with 
...
cross reference. We talk a bit about this later in this chapter when we introduce namespaces.
...
you chose in the beginning (_*.dmodel_). This will open the generated entity editor.
...
yield collisions as the model size increases. That's why most programming languages invent such things as namespaces or modules. We will now 
extend the language by the concept of a _Package_, which effectively is a namespace. 

Our languages's definition of a _Package_ is as follows: A package has a name and contains other named elements. A named element is either another 
...
of import statements, these names can be further abbreviated. That sounds all a bit complicated but the good thing is that Xtext comes with exactly such a default 
semantic for namespaces. Though, all aspects of this semantic are adaptable but for this tutorial we will stick with the defaults.

After we have successfully evolved our language, so that it allows to work with packages and imports, the following example should be valid syntax:
...
p. To allow qualified names, we need a new rule _QualifiedName_. This rule returns a string. But we do not want this to be a terminal rule, as terminal 
rules have special semantics (they are used contextless in the lexer) which often cause problems. Instead we want it to be a parser rule, but one which
...
calls go to either other _data type rules_ or other _terminal rules_. For a _data type rule_ the parser simply concats all the text it has consumed and returns it as a string.
...
which has been fully sufficient so far. But now we want to allow fully qualified names as well, so we explicitly specifiy the syntax rule after a delimiter '|':
...
p. As the last step, we introduce imports. An _Import_ is an _AbstractElement_, too, which means that it can occur as a child of _DomainModel_ as well as of _PackageDeclaration_.

bc.. AbstractElement:
	PackageDeclaration | Type | Import;

p. An imported namesapce is not just a qualified name but it also allows an optional wildcard characer '_*_' at the end, so that multiple names can be imported 
with one statement. That requires a new datatype rule _QualifiedNameWithWildCard_:
...
If such an _EAttribute_ is available, the default implementation computes a qualified name by asking the container for its name and concatenates the two names separated by a dot.
The computation of qualified names can be arbitrarily changed by implementing a _IQualifedNameProvider_. The other naming convention is that if some 
element has an EAttribute 'importedNamespace' that value is used as a namespace import and is automatically prefixed to any simple names used within that namespace.
...
p. Now Regenerate the language infrastructure as described in the previous section, and give the editor a try. You can even split up 
