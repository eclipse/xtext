...
*Note*: _This modules are replaced on every subsequent execution of the generator. Don't put any custom code into them. Manually written code has go into the "concrete subclasses":#guicemanualmodules._
...
p. In addition if the creation of the type causes any necessary side effects, so you want it to be instantiated eagerly, you can set the @eager@ property to true. This is shown in the following snippet:
