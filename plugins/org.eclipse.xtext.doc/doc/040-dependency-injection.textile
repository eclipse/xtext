...
Xtext is based on Dependency Injection. This means basically that whenever some code is in need for functionality (or state) 
...
class is instantiated, Guice sees that it needs an instance of IScopeProvider and sets it to the specified field or method.
This of course only works, if the object itself is created by Guice. In Xtext almost every object is created that way 
...
A _Module_ defines a set of mappings from types to either concrete instances, so called providers or
classes, which have a default constructor. Modules are implemented in Java, here's an example:

bc.. public class MyDslRuntimeModule extends org.xtext.example.mydsl.AbstractMyDslRuntimeModule {
	@Override
...
located in the ui bundle and adds and overrides bindings for running Xtext infrastructure within Eclipse. 
...
p. which would do the same as the code snippet above, which is declare a binding from _IScopeProvider_ to _MyConcreteScopeProvider_.
...
provide lazy access to a singleton or you need to to some computation each time an instance is created (i.e. factory). IF you want 
to point to a provider rather than to a concrete class you can use the following binding method.

bc.. public Class<? extends Provider<IScopeProvider>> provideIEncodingProvider() {
	return MyConcreteScopeProviderProvider.class;
}

p. _(Please forgive us the overuse of the term _provider_. The IScopeProvider is not a Guice _provider_.)_

p. That binding tells Guice to instantiate _MyConcreteTypeProvider_ and invoke get() in order to obtain an instance of _MyType_ 
for clients having declared a dependency to that type.
...
this something done programmatically in the main method. It could look like this:
...
of course want to levarage Guice also for all factories, etc. we put into those registries, we have introduced a so called
...
bc. Injector injector = new MyStandaloneSetup().createInjectorAndDoEMFRegistration(); 
