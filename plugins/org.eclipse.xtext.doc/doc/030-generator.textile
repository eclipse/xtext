...
p. one can create a family tree this way:

bc.. <x class="foo.Person">
  <name value="Grandpa"/>
...
As explained above the element _name_ '@x@' is ignored in this case. The attribute @class@
...
a single attribute @value@ is used and interpreted as if it was an 
attribute in the parent element. That is:

bc. <foo><name value="bar"/></foo>
...
*Tip*
__Whenever you are in an *.mwe file and wonder what kind of configuration the underlying component may accept: Just use JDT's open type action (CTRL+Shift+T) open the source file of the class in question, use the quick outline view (CTRL+O, use CTRL+O twice to see inherited members as well) and type 'set' or 'add' and you will see the available modifiers. Note that we plan to replace the XML syntax with an Xtext-based implementation as soon as possible.__
...
Now that you know a bit about MWE, you are ready to learn about the concepts and architecture of Xtext's language generator. 
...
As already explained we use MWE from EMFT in order to instantiate, configure and execute this structure of components. 
