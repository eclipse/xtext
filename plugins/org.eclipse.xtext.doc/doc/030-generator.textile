...
Xtext provides lots of generic implementations for you language's
...
plugin.xml, Manifest.MF and the "Guice modules":#Modules.
...
The method @Workflow.setBean()@ does nothing but provides a means to apply global side-effects, which unfortunately is required by some projects. In this example we do a so called __EMF stand alone setup__. This class initializes a bunch of things for a non-Equinox environment that are otherwise configured by means of extension points, e.g. EMF's @EPackage.Registry@.
