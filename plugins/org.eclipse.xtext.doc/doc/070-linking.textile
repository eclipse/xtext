...
Each string in a model which is assigned to an EAttribute with the name 'importURI', will be interpreted as a URI and used to be loaded using the ResourceSet of the current Resource.
...
p. It would be possible to write three files using in that language where the first references the other two, like this:
