...
** The "Transient Value":#transientvalues service could be used to indicate that this model element should not be consumed. 
...
*  For reasons of performance, it is critical that the Parse Tree Constructor takes the right branch first and detects wrong branches early. One way to achieve this is to avoid having many rules which return the same type and which are called from within the same grammar-alternative.   
...
p. Unassigned Text can be necessary due to data rule calls or terminal rule calls which do not reside within an assignment. Example:
