...
public class EclipseFileSystemAccessImpl implements FileSystemAccessImpl {
  @Inject
  ...
  public FolderHandle getSourceFolder(final CompilationUnitImpl it) {
    EclipseFolderHandle _xblockexpression = null;
    {
      final IPath sourcePath = this.getSourcePath(it);
      boolean _equals = Objects.equal(sourcePath, null);
      if (_equals) {
        return null;
      }
      IFile _file = this.getFile(it);
      IProject _project = _file.getProject();
      EclipseFolderHandle _eclipseFolderHandle = new EclipseFolderHandle(_project, sourcePath);
      _xblockexpression = (_eclipseFolderHandle);
    }
    return _xblockexpression;
  }
  
  public FolderHandle getRootFolder(final CompilationUnitImpl it) {
    EclipseFolderHandle _xblockexpression = null;
    {
      ...
      EclipseFolderHandle _eclipseFolderHandle = new EclipseFolderHandle(_project, _projectRelativePath);
      _xblockexpression = (_eclipseFolderHandle);
    }
    return _xblockexpression;
  }
  
  public FolderHandle getTargetFolder(final CompilationUnitImpl it) {
    EclipseFolderHandle _xblockexpression = null;
    {
      final IFile file = this.getFile(it);
      ...
      IProject _project = file.getProject();
      EclipseFolderHandle _eclipseFolderHandle = new EclipseFolderHandle(_project, path);
      _xblockexpression = (_eclipseFolderHandle);
    }
    return _xblockexpression;
  }
