...
public class CreateMemberQuickfixes implements ILinkingIssueQuickfixProvider {
	...
	protected boolean isThis(XAssignment assigment) {
		...
		return IFeatureNames.THIS.toString().equals(featureCall.getConcreteSyntaxFeatureName());
	}
	...
	protected JvmTypeReference getReceiverType(XAbstractFeatureCall featureCall) {
		...
		if (actualReceiver == null) {
			return typeRefs.createTypeRef(getCallersType(featureCall));
				...
				&& ((XAbstractFeatureCall) actualReceiver).isTypeLiteral()) {
	...
	protected String getOperatorMethodName(XAbstractFeatureCall call) {
		for (INode node : NodeModelUtils.findNodesForFeature(call,
	...
	protected void newLocalVariableQuickfix(final String variableName, XAbstractFeatureCall call, Issue issue,
		...
		issueResolutionAcceptor.accept(issue, "Create local variable '" + variableName + "'",
				...
				new SemanticModificationWrapper(issue.getUriToProblem(), new ISemanticModification() {
					public void apply(@Nullable final EObject element, @Nullable final IModificationContext context)
							throws Exception {
						if (element != null) {
							...
							if (xtendMember != null) {
								...
								if (offset != -1 && xtextDocument != null) {
									...
									appendable.increaseIndentation().newLine().append("val ").append(variableName)
											.append(" = ").append(defaultValueLiteral);
	...
	protected void newMethodQuickfixes(String newMemberName, XAbstractFeatureCall call, final Issue issue,
	...
	protected void newMethodQuickfixes(JvmTypeReference containerType, String name,
			@Nullable JvmTypeReference returnType, List<JvmTypeReference> argumentTypes, XAbstractFeatureCall call,
			JvmDeclaredType callersType, final Issue issue, final IssueResolutionAcceptor issueResolutionAcceptor) {
		...
		if (containerType.getType() instanceof JvmDeclaredType) {
			...
			newMethodQuickfix(declaredType, name, returnType, argumentTypes, isStatic, isAbstract, false, isLocal,
					call, issue, issueResolutionAcceptor);
		}
		if (!isLocal && !isStatic) {
			...
			newMethodQuickfix(callersType, name, returnType, extensionMethodParameterTypes, false, isAbstract, true,
					true, call, issue, issueResolutionAcceptor);
		}
	}

	protected void newMethodQuickfix(JvmDeclaredType containerType, String name, @Nullable JvmTypeReference returnType,
			List<JvmTypeReference> parameterTypes, boolean isStatic, boolean isAbstract, boolean isExtension,
			boolean isLocal, XAbstractFeatureCall call, final Issue issue,
			final IssueResolutionAcceptor issueResolutionAcceptor) {
	...
	protected void newGetterQuickfixes(String name, XAbstractFeatureCall call, final Issue issue,
			final IssueResolutionAcceptor issueResolutionAcceptor) {
		JvmDeclaredType callersType = getCallersType(call);
		JvmTypeReference receiverType = getReceiverType(call);
		JvmTypeReference fieldType = getNewMemberType(call);
		if (callersType != null && receiverType != null) {
			newMethodQuickfixes(receiverType, getAccessorMethodName("get", name), fieldType,
					Collections.<JvmTypeReference> emptyList(), call, callersType, issue, issueResolutionAcceptor);
		}
	}

	protected void newFieldQuickfix(String name, XAbstractFeatureCall call, final Issue issue,
			final IssueResolutionAcceptor issueResolutionAcceptor) {
		JvmDeclaredType callersType = getCallersType(call);
		JvmTypeReference receiverType = getReceiverType(call);
		JvmTypeReference fieldType = getNewMemberType(call);
	...
	protected void newFieldQuickfix(JvmDeclaredType containerType, String name, @Nullable JvmTypeReference fieldType,
			boolean isStatic, XAbstractFeatureCall call, final Issue issue,
			final IssueResolutionAcceptor issueResolutionAcceptor) {
