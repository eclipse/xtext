...
public class XtendHyperlinkHelper extends XbaseHyperLinkHelper {
	...
	public void createHyperlinksByOffset(XtextResource resource, int offset, IHyperlinkAcceptor acceptor) {
		...
		if (element instanceof XtendFunction) {
			...
			ILeafNode node = NodeModelUtils.findLeafNodeAtOffset(resource.getParseResult().getRootNode(), offset);
			if (isNameNode(member, XtendPackage.Literals.XTEND_FIELD__NAME, node) && member.getReturnType()==null) {
				EObject jvmElement = associations.getPrimaryJvmElement(member);
				if (jvmElement instanceof JvmIdentifiableElement) {
					addOpenInferredTypeHyperLink(resource, (JvmIdentifiableElement) jvmElement, node, acceptor);
				}
			}
			if (isNameNode(member, XtendPackage.Literals.XTEND_FIELD__NAME, node)) {
				JvmIdentifiableElement targetElement = (JvmIdentifiableElement) associations.getPrimaryJvmElement(element);
				...
				if (sourceViewer != null && javaElement != null && (javaElement.getElementType() == IJavaElement.METHOD && canBeOverridden((IMethod) javaElement))) {
					acceptor.accept(new XbaseImplementatorsHyperlink(javaElement, new Region(node.getOffset(), node.getLength()), sourceViewer, implOpener));
				}
				EObject jvmElement = associations.getPrimaryJvmElement(member);
				if (jvmElement instanceof JvmIdentifiableElement) {
					addOpenInferredTypeHyperLink(resource, (JvmIdentifiableElement) jvmElement, node, acceptor);
