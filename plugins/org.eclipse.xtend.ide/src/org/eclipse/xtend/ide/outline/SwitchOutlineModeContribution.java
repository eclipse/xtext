...
public class SwitchOutlineModeContribution extends AbstractToggleOutlineContribution {
	...
	protected void configureAction(Action action) {
		action.setText("Jvm Mode");
		action.setToolTipText("Jvm Mode");
		action.setDescription("Jvm Mode");
		action.setImageDescriptor(imageHelper.getImageDescriptor("full/etool16/java_app.gif"));
		action.setDisabledImageDescriptor(imageHelper.getImageDescriptor("full/etool16/java_app.gif"));
	}
	...
	protected void stateChanged(boolean newState) {
		if (outlinePage != null) {
			...
			if (treeProvider instanceof XtendOutlineTreeProvider) {
				((XtendOutlineTreeProvider) treeProvider).setJvmView(newState);
			}
