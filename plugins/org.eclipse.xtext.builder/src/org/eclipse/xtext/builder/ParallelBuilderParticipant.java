...
public class ParallelBuilderParticipant extends BuilderParticipant {
	...
	protected void doBuild(List<Delta> deltas, Map<String, OutputConfiguration> outputConfigurations,
		...
		BlockingQueue<Pair<IResourceDescription.Delta, Procedure2<? super IFileSystemAccess, ? super IProgressMonitor>>> requestQueue = newBlockingQueue(QUEUE_CAPACITY);
		...
		try {
			while (!requestQueue.isEmpty() || !removeDone(futures).isEmpty()) {
				...
				Delta delta = null;
				try {
					Pair<Delta, Procedure2<? super IFileSystemAccess, ? super IProgressMonitor>> pair = requestQueue.poll(QUEUE_POLL_TIMEOUT, TimeUnit.MILLISECONDS);
					if (pair != null) {
						delta = pair.getKey();
	...
	protected Runnable createRunnable(
			...
			final BlockingQueue<Pair<IResourceDescription.Delta, Procedure2<? super IFileSystemAccess, ? super IProgressMonitor>>> requestQueue, final IFileSystemAccess delegate) {
		return new Runnable() {
			...
			public void run() {
				try {
					...
					fileSystemAccessQueue.put(delta, getFlushAndCleanDerivedResourcesCallback(delta, derivedResources, context));
				} catch (Throwable e) {
					logErrorDuringCompilation(delta, e);
				}
			}
