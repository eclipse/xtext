...
public class ParallelBuilderParticipant extends BuilderParticipant {
	...
	protected void processSync(FileSystemAccessRequest request) {
		try {
			request.result = request.function.apply();
	...
	protected void processAsync(FileSystemAccessRequest request) {
		try {
			request.procedure.apply();
		} catch (OperationCanceledException e) {
			
		} catch (Exception e) {
			logErrorDuringCompilation(request.delta, e);
		}
	}
		...
		return new Runnable() {
			...
			public void run() {
				try {
					...
					fileSystemAccessQueue.sendAsync(delta, getFlushAndCleanDerivedResourcesCallback(delta, derivedResources, context, generated, delegate, progressMonitor));
				} catch (Throwable e) {
					logErrorDuringCompilation(delta, e);
				}
			}
