...
public class XtendFormatter extends XbaseFormatter {
	...
	def protected dispatch void format(RichString rs, extension IFormattableDocument format) {
		//		val (EObject, FormattableDocument)=>void callback = [EObject obj, IFormattableDocument doc|obj.format(doc)]
	...
	//	override protected isSingleLineBlock(XBlockExpression expr, extension IFormattableDocument format) {
	//		return format.cfg.get(keepOneLineMethods) && expr.eContainer instanceof XtendFunction &&
	//			expr.expressions.size <= 1 && !expr.isMultiline(format) &&
