...
public class TypeScopeWithWildcardImports extends AbstractScope {
	...
	public TypeScopeWithWildcardImports(List<ImportNormalizer> imports, AbstractTypeScope typeScope, AbstractScope parent) {
			...
			for(int i = 0; i < imports.length; i++) {
				...
				for(int j = 0; j < chunk.length; j++) {
					...
					if (namespacePrefix.equals(qualifiedPackageName)) {
						...
						result.add(EObjectDescription.create(withDot, type));
						if (withDollar != null) {
