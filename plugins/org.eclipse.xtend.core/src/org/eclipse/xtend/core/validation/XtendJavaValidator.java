...
import org.eclipse.xtext.xbase.typesystem.override.DefaultConflictResolvedOperation;
...
public class XtendJavaValidator extends XbaseWithAnnotationsJavaValidator {
		...
		for (IResolvedOperation operation : resolvedFeatures.getAllOperations()) {
			...
			if (operationDeclaringType != inferredType) {
				...
				if (visibilityHelper.isVisible(operation.getDeclaration())) {
					...
					if (operation instanceof DefaultConflictResolvedOperation
							&& contributesToConflict(inferredType, (DefaultConflictResolvedOperation) operation)
							&& !flaggedType) {
						IResolvedOperation conflictingOperation = ((DefaultConflictResolvedOperation) operation).getConflictingOperations().get(0);
	 ...
	 */
	private boolean contributesToConflict(JvmGenericType rootType, DefaultConflictResolvedOperation conflictResolvedOperation) {
		Set<JvmDeclaredType> involvedInterfaces = Sets.newHashSet();
		involvedInterfaces.add(conflictResolvedOperation.getDeclaration().getDeclaringType());
		for (IResolvedOperation conflictingOperation : conflictResolvedOperation.getConflictingOperations()) {
		...
		if (rootType.isInterface()) {
			...
			for (JvmTypeReference typeRef : rootType.getExtendedInterfaces()) {
				...
				if (rawType instanceof JvmDeclaredType && contributesToConflict((JvmDeclaredType) rawType, involvedInterfaces)) {
					contributingCount++;
				}
			}
			return contributingCount >= 2;
		} else {
			return contributesToConflict(rootType, involvedInterfaces);
		}
	}
