...
public class XtendJavaValidator extends XbaseWithAnnotationsJavaValidator {
	...
	protected void doCheckOverriddenMethods(XtendTypeDeclaration xtendType, JvmGenericType inferredType,
		...
		final boolean hasAtLeastTwoSuperInterfaces =
				xtendType instanceof XtendClass && ((XtendClass) xtendType).getImplements().size() >= 2
				|| xtendType instanceof XtendInterface && ((XtendInterface) xtendType).getExtends().size() >= 2;
		for (IResolvedOperation operation : resolvedFeatures.getAllOperations()) {
			...
			if (operationDeclaringType != inferredType) {
				...
				if (visibilityHelper.isVisible(operation.getDeclaration())) {
					...
					if (hasAtLeastTwoSuperInterfaces && declaredOperationsWithSameErasure.isEmpty()
							&& isInterface(operationDeclaringType)) {
