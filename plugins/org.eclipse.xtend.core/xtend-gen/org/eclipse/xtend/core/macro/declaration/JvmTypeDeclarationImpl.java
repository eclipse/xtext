...
public abstract class JvmTypeDeclarationImpl<T extends JvmDeclaredType> extends JvmMemberDeclarationImpl<T> {
  public Iterable<? extends MemberDeclaration> getDeclaredMembers() {
    return this.<MemberDeclaration>getDeclaredMembers(MemberDeclaration.class);
  }
  
  public <R extends MemberDeclaration> Iterable<? extends R> getDeclaredMembers(final Class<R> type) {
    T _delegate = this.getDelegate();
    EList<JvmMember> _members = _delegate.getMembers();
    final Function1<JvmMember,R> _function = new Function1<JvmMember,R>() {
      public R apply(final JvmMember it) {
        ...
        return ((R) _memberDeclaration);
      }
    };
    List<R> _map = ListExtensions.<JvmMember, R>map(_members, _function);
    return ImmutableList.<R>copyOf(_map);
  }
