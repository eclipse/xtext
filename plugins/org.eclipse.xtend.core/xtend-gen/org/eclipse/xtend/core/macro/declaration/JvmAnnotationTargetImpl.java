...
public abstract class JvmAnnotationTargetImpl<T extends JvmAnnotationTarget> extends JvmNamedElementImpl<T> {
  public Iterable<? extends AnnotationReference> getAnnotations() {
    return this.<AnnotationReference>getAnnotations(AnnotationReference.class);
  }
  
  public <R extends AnnotationReference> Iterable<? extends R> getAnnotations(final Class<R> type) {
    T _delegate = this.getDelegate();
    EList<JvmAnnotationReference> _annotations = _delegate.getAnnotations();
    final Function1<JvmAnnotationReference,R> _function = new Function1<JvmAnnotationReference,R>() {
      public R apply(final JvmAnnotationReference it) {
        ...
        return ((R) _annotationReference);
      }
    };
    List<R> _map = ListExtensions.<JvmAnnotationReference, R>map(_annotations, _function);
