...
public class CachingResourceValidatorImpl extends DerivedStateAwareResourceValidator {
  ...
  protected List<Issue> doValidate(final Resource resource, final CheckMode mode, final CancelIndicator mon) {
    ...
    try {
      ...
      if (_isCanceled) {
        return Collections.<Issue>unmodifiableList(Lists.<Issue>newArrayList());
      }
  ...
  private void runActiveAnnotationValidation(final Resource resource, final CancelIndicator monitor) {
    ...
    try {
      ActiveAnnotationContexts _computeContext = this.contextProvider.computeContext(file);
      contexts = _computeContext;
    } catch (final Throwable _t) {
      if (_t instanceof Throwable) {
        final Throwable t = (Throwable)_t;
        this.handleProcessorInitializationError(file);
        return;
      } else {
        throw Exceptions.sneakyThrow(_t);
      }
    ...
    for (final ActiveAnnotationContext ctx : _values) {
      {
        ...
        try {
          ...
          if (_t_1 instanceof Throwable) {
            ...
            ctx.handleProcessingError(_eResource, t_1);
          } else {
    ...
    this.addWarningsForOrphanedJvmElements(file, monitor);
  }
  
  private boolean handleProcessorInitializationError(final XtendFile file) {
    Resource _eResource = file.eResource();
    EList<Resource.Diagnostic> _errors = _eResource.getErrors();
    EObjectDiagnosticImpl _eObjectDiagnosticImpl = new EObjectDiagnosticImpl(
      Severity.ERROR, 
      IssueCodes.PROCESSING_ERROR, 
      "Could not create active annotation contexts", file, 
      XtendPackage.Literals.XTEND_FILE__PACKAGE, 
      (-1), 
      null);
    return _errors.add(_eObjectDiagnosticImpl);
  }
  
  private void addWarningsForOrphanedJvmElements(final XtendFile file, final CancelIndicator monitor) {
    Resource _eResource = file.eResource();
      ...
      for (final JvmMember jvmMember : _iterable) {
        {
          ...
          final EObject sourceElement = this.associations.getPrimarySourceElement(jvmMember);
          boolean _tripleEquals = (sourceElement == null);
          if (_tripleEquals) {
            this.addWarningForOrphanedJvmElement(file, jvmMember);
          }
        }
      }
    }
  ...
  private boolean addWarningForOrphanedJvmElement(final XtendFile file, final JvmMember jvmElement) {
    Resource _eResource = file.eResource();
    ...
    EObjectDiagnosticImpl _eObjectDiagnosticImpl = new EObjectDiagnosticImpl(
      Severity.WARNING, 
      IssueCodes.ORPHAN_ELMENT, _builder.toString(), file, 
      XtendPackage.Literals.XTEND_FILE__PACKAGE, 
      (-1), 
      null);
    return _warnings.add(_eObjectDiagnosticImpl);
  }
