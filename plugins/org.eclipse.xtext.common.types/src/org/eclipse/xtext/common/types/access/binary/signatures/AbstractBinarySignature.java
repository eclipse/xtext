...
public abstract class AbstractBinarySignature {
	...
	protected AbstractBinarySignature(char[] chars) {
		this(chars, 0, chars.length);
	}

	protected AbstractBinarySignature(char[] chars, int offset, int length) {
		this.chars = chars;
		this.offset = offset;
		this.length = length;
				...
				while (chars[i] == ':') {
					...
					switch (chars[i]) {
						...
						case Signature.C_RESOLVED:
							try {
								i = Util.scanClassTypeSignature(chars, i);
						...
						case Signature.C_ARRAY:
							try {
								i = Util.scanArrayTypeSignature(chars, i);
								i++; // position at start of next param if any
							} catch (IllegalArgumentException e) {
						...
						case Signature.C_TYPE_VARIABLE:
							try {
								i = Util.scanTypeVariableSignature(chars, i);
								i++; // position at start of next param if any
							} catch (IllegalArgumentException e) {
