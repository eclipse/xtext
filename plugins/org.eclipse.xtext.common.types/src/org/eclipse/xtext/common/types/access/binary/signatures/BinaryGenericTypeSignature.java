...
public class BinaryGenericTypeSignature extends BinaryTypeSignature {
	...
	protected BinaryGenericTypeSignature(char[] chars) {
		super(chars);
		checkInvariant();
	}
	
	protected BinaryGenericTypeSignature(char[] chars, int offset, int length) {
	...
	public List<BinaryTypeArgumentSignature> getTypeArguments() {
		...
		while (true) {
			...
			int end = Util.scanTypeArgumentSignature(chars, p);
