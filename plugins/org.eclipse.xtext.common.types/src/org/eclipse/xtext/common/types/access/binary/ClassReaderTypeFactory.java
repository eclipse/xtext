...
public class ClassReaderTypeFactory implements ITypeFactory<BinaryClass> {
	 ...
	 * It's demand populated when {@link #createProxy(BinaryTypeSignature, Map) creating} a type proxy.
	 */
	 ...
	 * It's demand populated when {@link #createMethodProxy(BinaryTypeSignature, char[]) creating} a method proxy.
	 */
	...
	protected JvmDeclaredType doCreateType(BinaryClass binaryClass, boolean allowInner, Map<String, JvmTypeParameter> typeParameters) throws ClassFormatException {
	...
	/**
	 * @param tagBits a bit mask that is used to encode standard annotation values, e.g. {@code @Target} or {@code @Deprecated} 
	 */
	protected void createAnnotationValues(final IBinaryAnnotation[] annotations, long tagBits, final JvmAnnotationTarget result) {
		if (annotations != null) {
	...
	private static final BinaryTypeSignature JAVA_LANG_SAFEVARARGS = BinarySignatures.createTypeSignature(CharOperation.concatWith(TypeConstants.JAVA_LANG_SAFEVARARGS, '/'));
	...
	private JvmAnnotationReference generateStandardAnnotation(BinaryTypeSignature annotationType, List<JvmEnumerationLiteral> enumLiterals) {
	...
	private List<JvmEnumerationLiteral> getRetentionPolicy(long tagBits) {
		...
		if ((tagBits & TagBits.AnnotationRuntimeRetention) == TagBits.AnnotationRuntimeRetention) {
			...
			return Collections.singletonList(createEnumLiteralProxy(TypeConstants.UPPER_RUNTIME, JAVA_LANG_ANNOTATION_RETENTIONPOLICY));
		} else if ((tagBits & TagBits.AnnotationSourceRetention) != 0) {
			return Collections.singletonList(createEnumLiteralProxy(TypeConstants.UPPER_SOURCE, JAVA_LANG_ANNOTATION_RETENTIONPOLICY));
		} else {
			return Collections.singletonList(createEnumLiteralProxy(TypeConstants.UPPER_CLASS, JAVA_LANG_ANNOTATION_RETENTIONPOLICY));
		}
	}
		...
		if (values.length >= 1) {
			...
			for(int i = 1; i < values.length; i++) {
				...
				if (value instanceof Constant) {
					...
					switch (c.typeID()) {
						case TypeIds.T_int: {
							((InternalEList<Integer>) ((JvmIntAnnotationValue) result).getValues()).addUnique(c.intValue());
							break;
						}
						case TypeIds.T_byte: {
							((InternalEList<Byte>) ((JvmByteAnnotationValue) result).getValues()).addUnique(c.byteValue());
							break;
						}
						case TypeIds.T_short: {
							((InternalEList<Short>) ((JvmShortAnnotationValue) result).getValues()).addUnique(c.shortValue());
							break;
						}
						case TypeIds.T_char: {
							((InternalEList<Character>) ((JvmCharAnnotationValue) result).getValues()).addUnique(c.charValue());
							break;
						}
						case TypeIds.T_float: {
							((InternalEList<Float>) ((JvmFloatAnnotationValue) result).getValues()).addUnique(c.floatValue());
							break;
						}
						case TypeIds.T_double: {
							((InternalEList<Double>) ((JvmDoubleAnnotationValue) result).getValues()).addUnique(c.doubleValue());
							break;
						}
						case TypeIds.T_boolean: {
							((InternalEList<Boolean>) ((JvmBooleanAnnotationValue) result).getValues()).addUnique(c.booleanValue());
							break;
						}
						case TypeIds.T_long: {
							((InternalEList<Long>) ((JvmLongAnnotationValue) result).getValues()).addUnique(c.longValue());
							break;
						}
						case TypeIds.T_JavaLangString: {
							((InternalEList<String>) ((JvmStringAnnotationValue) result).getValues()).addUnique(c.stringValue());
							break;
						}
					}
				}
				if (value instanceof ClassSignature) {
					...
					((InternalEList<JvmTypeReference>) ((JvmTypeAnnotationValue) result).getValues()).addUnique(createTypeReference(BinarySignatures.createTypeSignature(typeName), null));
				} else if (value instanceof IBinaryAnnotation) {
					((InternalEList<JvmAnnotationReference>) ((JvmAnnotationAnnotationValue) result).getValues()).addUnique(createAnnotationReference((IBinaryAnnotation) value));
				} else if (value instanceof EnumConstantSignature) {
					EnumConstantSignature ref = (EnumConstantSignature) value;
					JvmEnumerationLiteral enumLiteralProxy = createEnumLiteralProxy(ref.getEnumConstantName(), ref.getTypeName());
	...
	protected JvmAnnotationValue createAnnotationValue(Object value) {
		...
		if (value instanceof IBinaryAnnotation) {
			...
			((InternalEList<JvmAnnotationReference>) result.getValues()).addUnique(createAnnotationReference((IBinaryAnnotation) value));
			return result;
		}
	...
	protected void setSuperTypes(IBinaryType classFile, BinarySuperTypeSignature genericSignature, JvmDeclaredType result, Map<String, JvmTypeParameter> typeParameters) {
		...
		if (genericSignature != null) {
			...
			if (superType != null) {
				superTypes.addUnique(createTypeReference(BinarySignatures.createTypeSignature(superType), typeParameters));
			}
	...
	protected void setInterfaces(IBinaryType classFile, Map<String, JvmTypeParameter> typeParameters,
		...
		if (interfaceNames != null) {
			for(int i = 0; i < interfaceNames.length; i++) {
				...
				result.addUnique(createTypeReference(BinarySignatures.createTypeSignature(interfaceName), typeParameters));	
			}
		}
	}
	...
	protected void createFields(IBinaryType classFile, JvmDeclaredType result, Map<String, JvmTypeParameter> typeParameters) {
	...
	protected void createConstructors(IBinaryType classFile, JvmDeclaredType result, int offset, Map<String, JvmTypeParameter> typeParameters) {
	...
	protected void createMethods(IBinaryType classFile, JvmDeclaredType result, Map<String, JvmTypeParameter> typeParameters) {
		IBinaryMethod[] declaredMethods = classFile.getMethods();
		if (declaredMethods != null) {
			InternalEList<JvmMember> members = (InternalEList<JvmMember>)result.getMembers();
			for (int i = 0; i < declaredMethods.length; i++) {
	...
	protected Map<String, JvmTypeParameter> createTypeParameters(AbstractBinarySignature signature, JvmTypeParameterDeclarator container, Map<String, JvmTypeParameter> knownTypeParameters) {
		...
		if (typeParameters.size() != 0) {
			Map<String, JvmTypeParameter> result = Maps.newHashMapWithExpectedSize(
					(knownTypeParameters != null ? knownTypeParameters.size() : 0) + typeParameters.size());
			if (knownTypeParameters != null)
				result.putAll(knownTypeParameters);
			InternalEList<JvmTypeParameter> jvmTypeParameters = (InternalEList<JvmTypeParameter>)container.getTypeParameters();
			...
			for(int i = 0; i < typeParameters.size(); i++) {
				...
				InternalEList<JvmTypeConstraint> constraints = (InternalEList<JvmTypeConstraint>)typeParameter.getConstraints();
	...
	protected JvmTypeReference createTypeArgument(BinaryTypeArgumentSignature actualTypeArgument, Map<String, JvmTypeParameter> typeParameters) {
	...
	protected JvmField createField(IBinaryField field, String enclosingTypeName, Map<String, JvmTypeParameter> typeParameters) {
		JvmField result;
	...
	protected JvmConstructor createConstructor(IBinaryMethod method, int offset, JvmDeclaredType declarator, Map<String, JvmTypeParameter> typeParameters) {
	...
	protected void enhanceExecutable(JvmExecutable result, 
			IBinaryMethod method,
			BinaryMethodSignature binarySignature,
			JvmDeclaredType owner,
			int offset,
			...
			for(int i = offset; i <parameterTypes.size(); i++) {
				...
				parameters.addUnique(
						createFormalParameter(
								parameterType, 
								hasArgumentNames ? String.valueOf(argumentNames[i - offset]) : "arg" + (i - offset),
								result,
								method,
		...
		if (exceptions.size() == 0) {
			...
			if (rawExceptionTypes != null) {
				...
				for(int i = 0, max = rawExceptionTypes.length; i < max; i++) {
					resultExceptions.addUnique(createTypeReference(BinarySignatures.createTypeSignature(rawExceptionTypes[i]), typeParameters));
				}
			}
		} else {
			InternalEList<JvmTypeReference> resultExceptions = (InternalEList<JvmTypeReference>)result.getExceptions();
	...
	protected Map<String, JvmTypeParameter> enhanceGenericDeclaration(JvmExecutable result, BinaryMethodSignature signature, Map<String, JvmTypeParameter> typeParameters) {
		return createTypeParameters(signature, result, result.isStatic() ? null : typeParameters);
	}

	protected JvmOperation createOperation(IBinaryMethod method, JvmDeclaredType declaringClass, Map<String, JvmTypeParameter> typeParameters) {
	...
	protected JvmFormalParameter createFormalParameter(
			BinaryTypeSignature parameterType,
			String paramName,
			JvmMember container,
			IBinaryMethod method, 
