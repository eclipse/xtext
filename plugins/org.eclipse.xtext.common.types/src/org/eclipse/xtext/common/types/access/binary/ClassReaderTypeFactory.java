...
public class ClassReaderTypeFactory implements ITypeFactory<BinaryClass> {
	...
	@Inject
	public ClassReaderTypeFactory(SignatureURIHelper uriHelper, ClassLoader loader) {
	...
	protected ClassFileReader getBinaryType(BinaryClass binaryClass) throws ClassFormatException {
		ClassFileReader result = new ClassFileReader(binaryClass.getBytes(), binaryClass.getName().toCharArray(), true);
		return result;
	}
	...
	public JvmDeclaredType createType(IBinaryType classFile, Map<String, JvmTypeParameter> typeParameters) {
		...
		createAnnotationValues(classFile.getAnnotations(), result);
		return result;
	}
	...
	protected void createAnnotationValues(final IBinaryAnnotation[] annotations, final JvmAnnotationTarget result) {
		if (annotations != null) {
	...
	protected JvmEnumerationLiteral createEnumLiteralProxy(char[] literalName, char[] typeName) {
		...
		BinarySimpleMemberSignature fieldSignature = BinarySignatures.createTypeSignature(typeName).appendField(literalName);
	...
	protected void setSuperTypes(IBinaryType classFile, BinarySuperTypeSignature genericSignature, JvmDeclaredType result, Map<String, JvmTypeParameter> typeParameters) {
		...
		if (genericSignature != null) {
			...
			if (superTypes.isEmpty() && !Object.class.getName().equals(classFile.getName())) {
	...
	protected JvmTypeReference createObjectTypeReference() {
		return createTypeReference(BinarySignatures.createTypeSignature("java/lang/Object".toCharArray()), null);
	}
		...
		if (defaultValue != null) {
			if (defaultValue instanceof Object[]) {
				...
				if (castedDefaultValue.length == 0) {
					...
					if (returnType.isSignature("[Ljava/lang/Class;")) {
	...
	protected JvmTypeReference createTypeReference(BinaryTypeSignature type, Map<String, JvmTypeParameter> typeParameters) {
		...
		if (typeArguments.size() == 0) {
			...
			result.setType(createProxy(type, typeParameters));
			return result;
		} else {
			JvmParameterizedTypeReference result = TypesFactory.eINSTANCE.createJvmParameterizedTypeReference();
	...
	protected JvmField createField(IBinaryField field, String enclosingTypeName, Map<String, JvmTypeParameter> typeParameters) {
		...
		createAnnotationValues(field.getAnnotations(), result);
		return result;
	}
	...
	protected JvmConstructor createConstructor(IBinaryMethod method, int offset, JvmDeclaredType declarator, Map<String, JvmTypeParameter> typeParameters) {
		...
		createAnnotationValues(method.getAnnotations(), result);
		return result;
	}
	...
	protected void enhanceExecutable(JvmExecutable result, 
		...
		if (method.isConstructor()) {
			...
			result.setSimpleName(String.valueOf(method.getSelector()));
		}
		fqName.append('(');
	...
	protected SignatureURIHelper getUriHelper() {
		return uriHelper;
	}
