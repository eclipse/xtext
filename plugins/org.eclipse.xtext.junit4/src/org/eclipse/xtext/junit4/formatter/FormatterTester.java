...
import org.eclipse.xtext.formatting2.internal.ReplacementsToString;
...
public class FormatterTester {
	...
	public void assertFormatted(FormatterTestRequest req) {
		...
		NodeModelBaseRegionAccess tokenAccess = nodeModelTokenAccessBuilderProvider.get().withResource(parsed).create();
	...
	protected void assertReplacementsAreInRegion(List<ITextReplacement> rep, ITextRegion region, String document) {
		...
		for (ITextReplacement r : rep)
			if (r.getOffset() < offset)
				...
				invalid.add(r);
		if (!invalid.isEmpty()) {
			String visualized = new ReplacementsToString<ITextReplacement>().apply(invalid);
