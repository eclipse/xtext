...
\title{Xbase Language Specification}
...
the main expression \lstinline[]°e° is evaluated first and then each case sequentially. The result of the main expression, further called the switch value, is bound (and possibly shadows) the implicit
\lstinline[]°this° variable. In order to avoid shadowing, a 
\hyperref[VariableDeclaration]{variable declaration~(§\ref*{VariableDeclaration})} can be used.

A case clause is taken if the switch value equals the result of the case's guard expression or if the case's guard expression evaluates to \lstinline[]°true°.


\subsection{Leaving out the main expression}

It is possible to leave out the main expression. Then the first case clause for which its guard returns \lstinline[]°true° is taken and executed.
...
Only if the switch value passes a type guard, i.e. an instanceof operation returns \lstinline[]°true°, the case's guard expression is executed using the same semantics explained in
...
\autoref{CommonSuperType}. The set of types from which the common super type is 
...
\hyperref[Blocks]{blocks~(§\ref*{Blocks})} and 
\hyperref[SwitchExpression]{switch expressions~(§\ref*{SwitchExpression})}. They are visible in any subsequent expressions in
the block.
...
The return type of a block is also the type of the last expression. Empty blocks return \lstinline[]°null°. 
...
\noindent\url{http://java.sun.com/docs/books/jls/third_edition/html/lexical.html#3.7}

\noindent\url{http://java.sun.com/docs/books/jls/third_edition/html/j3TOC.html}

\noindent\url{http://java.sun.com/docs/books/jls/third_edition/html/lexical.html#3.8}

\noindent\url{http://java.sun.com/docs/books/jls/third_edition/html/lexical.html#3.10.5}

\noindent\url{http://www.scala-lang.org/docu/files/ScalaReference.pdf}
