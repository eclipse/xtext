...
This document specifies the expression language library Xbase. Xbase is a partial programming language implemented in Xtext and is meant to be embedded and extended
...
statemachines can be developed in minutes. However, software systems do not consist of structures solely. At some point a system needs to do something, hence
we want to specify some behavior which is usually done using so called \textit{expressions}. Expressions are the heart of every programming language and are not so easy to get right.
...
returns true. For the other characters also digits (0-9) are allowed. For those the method \lstinline°Character.isJavaIdentifierPart(char)° must be true.
...
conflict with a reserved keyword. Imagine you have introduced a keyword 'service' in your language but want at some point call a Java property 'service'. 
...
The compiler makes sure that only numbers between 0 and Integer.MAX (0x7fffffff) are used.

There's no negative integer literal, instead the expression \lstinline°-23° is parsed as the prefix operator \lstinline°-° applied on an int literal.
...
\section{White Space}
\label{Whitespace}
The white space characters ' ','\textbackslash{}t','\textbackslash{}n' and '\textbackslash{}r' are allowed to occure anywhere between the other syntactic elements. 
...
wrapper type, that is \textit{java.lang.Boolean} instead of \textit{boolean}. Also Arrays are supported in Xbase but are translated to a \textit{java.util.List}. For example an 
array \textit{int[]} binds to \textit{java.util.List<java.lang.Integer>} (in short \textit{List<Integer>}). 
This means when refering to \lstinline°myList.isEmpty()° within an Xbase expression the static return type is \textit{java.lang.Boolean}. 
...
\label{VoidType}
The \lstinline°null° reference is the only valid value of the type \lstinline°Void°, which gets some special treatment in Xbase. That is every Java method which is declared \textit{void} (i.e. without a return value)
...
be statically typed. That is by default it is assumed that languages making use of Xbase provide enough static context information for static type analysis, which is the basis of a lot of
...
to operations on certain types, but instead Xbase comes with an operator to method mapping, which allows users to redefine the 
...
The table above also defines the operator precedence (from low to high precedence). The separator lines indicate a precedence level. 
...
If there's no such method, the compiler looks for a method called \lstinline°addMyProperty(OneArg)° on the type of the target expression
...
in short circuit mode, which means that the right hand operand might not be evaluated at all in certain cases.
Such cases are: 
...
the compiler first looks for an accessible field in the type of myObj. If no such field exists it looks for a method called \lstinline°myProperty()° before it looks 
...
\section{Closures}
\label{Closures}
A closure is a literal to define anonymous functions. A closure also captures the current scope, so that any final variables and parameters visible at construction time 
...
An Xbase closure is a Java object of one of the Function interfaces shipped with the runtime library of Xbase. There's an interface for each number of parameters.
...
There's also an auto-coercion for any \lstinline°Function1<T,Boolean>° to \lstinline°com.google.common.base.Predicate<T>°.
...
closures also to methods expecting other such types like e.g. \lstinline°Iterable°. Also this would avoid a fixed dependency to Google Guava.}
...
They are like a combination of the if statements and the ternary operator (\lstinline°predicate ? thenPart : elsePart°) in Java. This means they use the syntax of Java's if statement,
but are eactually expressions like Java's ternary operator, hence they always return something and have a compile-time type.
This allows to use if clauses deeply nested within expressions.
...
first the main expression \lstinline°e° is evaluated and then each case sequentially. 
If a case expression \lstinline°en° evaluates to something so that \lstinline°e == en° the result of the switch expression is \lstinline°ern°. If none of the 
case expressions \lstinline°e1...en° was equal to the result of the main expression \lstinline°e°, the result of the optional default part \lstinline°er° is returned.
...
The expression x, will be of type \lstinline°String° in any expression within the first case and of type \lstinline°List<?>° in the second. Not that the case expression
...
Note that the expression x is reevaluated everytime it is used. If the expression returns a different value on a subsequent evaluation, it might cause 
...
\autoref{CommonSuperType}. 
The set of types from which the common super type is computed correpsonds to the types of each case's result expression.
...
The type of the variable itself can either be explcitly declared or be inferred from the right hand side expression.
...
It can optionally be terminated with a semicolon.
...
A while loop \lstinline°while (predicate) expression° is used to execute a certain expression unless a given predicate is evaluated to \lstinline°false°.
...
A do-while loop \lstinline°do e while (p)° is used to execute a certain expression \lstinline°e° unless a given predicate \lstinline°p° is evaluated to \lstinline°false°.
...
\section{For Loop}
\label{ForLoop}
The for loop \lstinline°for (T1 var : iterableOfT1) expression° is used to execute a certain expression for each element of an \lstinline°java.lang.Iterable°.
The local variable is final, hence canot be updated. 
