...
@GwtCompatible public class MapExtensions {
	...
	private static <K, V> Map<K, V> cloneMap() {
	 ...
	 * @since 2.8
	 */
	public static <K, V> Map<K, V> operator_remove(Map<K, V> map, Pair<K, V> pair) {
		V value = map.get(pair.getKey());
		if (Objects.equals(value, pair.getValue())) {
	 ...
	 * @since 2.8
	 */
	public static <K, V> Map<K, V> operator_remove(Map<K, V> map, Map<K, V> pairsToRemove) {
		for (Entry<K, V> pair : pairsToRemove.entrySet()) {
			V value = map.get(pair.getKey());
			if (Objects.equals(value, pair.getValue())) {
				map.remove(pair.getKey());
			}
	 ...
	 * @since 2.8
	 */
	public static <K, V> Map<K, V> operator_remove(Map<K, V> map, Iterable<K> keysToRemove) {
	 ...
	 */
	@Pure
	public static <K, V> Map<K, V> operator_minus(Map<K, V> map, Pair<K, V> pair) {
		Map<K, V> newMap = cloneMap();
		for (Entry<K, V> opair : map.entrySet()) {
			if (!Objects.equals(pair.getKey(), opair.getKey())
				|| !Objects.equals(pair.getValue(), opair.getValue())) {
	 ...
	 */
	@Pure
	public static <K, V> Map<K, V> operator_minus(Map<K, V> map1, Map<K, V> map2) {
		Map<K, V> newMap = cloneMap();
		for (Entry<K, V> opair : map1.entrySet()) {
			V value = map2.get(opair.getKey());
			if (!Objects.equals(value, opair.getValue())) {
				newMap.put(opair.getKey(), opair.getValue());
			}
		}
		return newMap;
	 ...
	 */
	@Pure
	public static <K, V> Map<K, V> operator_minus(Map<K, V> map, Iterable<K> pairs) {
		Map<K, V> newMap = cloneMap();
		Set<K> keys = null;
		for (Entry<K, V> opair : map.entrySet()) {
			if (keys == null) {
				...
				for (K key : pairs) {
					keys.add(key);
					if (Objects.equals(key, opair.getKey())) {
						isFound = true;
					}
				}
				if (!isFound) {
