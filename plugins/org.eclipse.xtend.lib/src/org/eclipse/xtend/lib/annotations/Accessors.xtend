...
class AccessorsProcessor implements TransformationParticipant<MutableMemberDeclaration> {
	...
	static class Util {
		...
		def getPossibleGetterNames(FieldDeclaration it) {
			(if(type.orVoid.isBooleanType) #["is", "get"] else #["get"]).map[prefix|prefix + simpleName.toFirstUpper]
		}
		...
		def hasSetter(FieldDeclaration it) {
			declaringType.findDeclaredMethod(setterName, type.orVoid) !== null
		}
		...
		def void addSetter(MutableFieldDeclaration field, Visibility visibility) {
			...
			field.declaringType.addMethod(field.setterName) [
				...
				val param = addParameter(field.simpleName, field.type.orVoid)
