...
## Xtend and Java 17
Xtend now support Java 17 as source and target too. This requires newer Eclipse platform and JDT jars that no longer run on Java 8. Thus Xtend 2.29 requires Java 11 as minimal Java version. While we were at we also bumped the minimal tested Eclipse version to 2022-03 (coming from Oxygen). Please Note: We did no special treatment for newer Java constructs in Xtend, so what works, works, what does not, does not. Please feel free to file bugs (and PRs)
