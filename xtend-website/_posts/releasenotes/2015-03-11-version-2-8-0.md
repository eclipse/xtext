...
The full resolution of Xtend resources can be very expensive. During compilation, the compiler goes through different phases that involve a lot of computation. With 2.8 we store the computed resource state and load it on demand if no changes are expected. This reduces the amount of work for the builder on incremental changes significantly. The API is still provisional for now.
